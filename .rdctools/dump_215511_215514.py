...
EIDS = [3379, 3383]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215511_215514.json'
CBS = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215511_cbs.json'
INSTANCE_NAMES = ('uniforms30', 'uniforms24')
...
def work(c):
    ...
    for eid in EIDS:
        ...
        def stage_cbs(stage, key):
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if name in INSTANCE_NAMES and sz >= 256:
                    ...
                    extra = {}
                    if len(raw) >= 256:
                        extra['child1'] = floats(raw[64:80])
                        extra['child2'] = floats(raw[80:96])
                        extra['child4'] = floats(raw[160:176])
                    ...
                    rec['sha'] = hexhash(raw)[:16]
                    if name in ('uniforms43', 'uniforms45', 'uniforms47', 'uniforms21', 'uniforms27') or sz in (16, 32, 208, 656, 3200) or name in ("uniforms43","uniforms45","uniforms47"):
                        rec_floats = floats(raw[:min(sz, 512)])
                        cbs['%s_%s_%d' % (key, name, eid)] = {
                            'size': sz,
                            'sha': rec['sha'],
                            'floats': rec_floats,
                        }
                        if sz == 3200 and len(raw) >= 440:
                            cbs['%s_%s_%d' % (key, name, eid)]['mip416'] = floats(raw[416:420])[0]
                            cbs['%s_%s_%d' % (key, name, eid)]['y436'] = floats(raw[436:440])[0]
                out.append(rec)
    ...
    return {
        ...
        'srefPer': [x['sref'] for x in draws],
        'zwPer': [x['zw'] for x in draws],
        'cullPer': [x['cull'] for x in draws],
    }
