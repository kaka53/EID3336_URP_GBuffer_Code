...
def work(c):
    ...
    shader_files = {
        'VS215523': {
            'spv': save(os.path.join(SH, 'VS215523.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS215523.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS215524': {
            'spv': save(os.path.join(SH, 'PS215524.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS215524.spvasm'), ps_asm),
    ...
    out = {
        ...
        'notes': {
            ...
            'vsSha256': shader_files['VS215523']['sha256'],
            'psSha256': shader_files['PS215524']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215523_PS215524_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
