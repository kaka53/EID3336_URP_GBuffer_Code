...
EIDS = [3589, 3593]
FAMILY = 'VS218872_PS218873'
EXPECTED = (218872, 218873)
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS218872_PS218873_Batch'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        if raster is None:
            ...
            raster = {
                ...
                'stencilFront': str(st[0]) if st else '',
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if inst_cb and sz >= 256:
                    take = int(a.numInstances) * 256
                    raw = getbuf(d.resource, d.byteOffset, take)
                elif lut:
                    take = 16
                    raw = getbuf(d.resource, d.byteOffset, 16)
                else:
                    take = sz
    ...
    shader_files = {}
    for stage, key in [(rd.ShaderStage.Vertex, 'VS218872'), (rd.ShaderStage.Pixel, 'PS218873')]:
        refl = s.GetShaderReflection(stage)
        shader_files[key] = {
    ...
    existing = {}
    search_roots = [
        os.path.join(ASSETS, 'ColourPass6_VS218872_PS218873_Batch', 'TextureDatabase'),
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms27 / PS uniforms22 stride 256',
            'localMaterialCB': 'PS uniforms30 / VS uniforms36 352B',
            'skinning': 'flags bit32 all zero; no rw ssbo',
            'raster': raster,
            'liveUnityVP': True,
    ...
    open(os.path.join(ROOT, 'VS218872_PS218873_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_218872_218873_batch_result.json'
    summary = {
