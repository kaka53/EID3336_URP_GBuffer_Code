...
EIDS = [3771, 3776]
FAMILY = 'VS215839_PS215840'
EXPECTED = (215839, 215840)
INSTANCE_STRIDE = 96
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215839_PS215840_Batch'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if key == 'VS' and name == 'uniforms25':
                    ...
                    instance_file = save(os.path.join(INST, 'EID%d_uniforms25_%d.bytes' % (eid, instance_count)), raw)
        ...
        if instance_file is None:
            raise RuntimeError('EID%d missing VS uniforms25 instance array' % eid)
    ...
    shader_files = {
        'VS215839': {
            'spv': save(os.path.join(SH, 'VS215839.spv'), open(os.path.join(shader_src, '96360155ef3e6bb7_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS215839.spvasm'), open(os.path.join(shader_src, '96360155ef3e6bb7_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, '96360155ef3e6bb7_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS215840': {
            'spv': save(os.path.join(SH, 'PS215840.spv'), open(os.path.join(shader_src, '3fb447c54784406d_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS215840.spvasm'), open(os.path.join(shader_src, '3fb447c54784406d_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, '3fb447c54784406d_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    out = {
        ...
        'requestedEIDs': EIDS,
        'notes': {
            'vsSha256': shader_files['VS215839']['sha256'],
            'psSha256': shader_files['PS215840']['sha256'],
            'instanceStride': INSTANCE_STRIDE,
    ...
    open(os.path.join(ROOT, 'VS215839_PS215840_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215839_215840_batch_result.json'
    summary = {
