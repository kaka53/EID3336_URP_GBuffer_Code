...
EIDS = [3844]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215843.json'
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                mip = (sz == 3200)
                # Do not assume instance CB names. Dump full small CBs; slice CBs
                # that look like instance arrays (sz >= n*96 and sz % 16 == 0).
                looks_inst = False
                if n > 0 and sz >= 96:
                    for stride in (96, 256):
                        if sz >= n * stride and (sz == n * stride or sz >= 65536):
                            looks_inst = True
                            rec['slices']['%s_%s_guess_stride' % (key, name)] = stride
                            raw = bytes(c.GetBufferData(d.resource, int(d.byteOffset), n * stride))
                            ...
                            if key == 'VS':
                                rec['flags'] = [struct.unpack_from('<I', raw, ii * stride + (76 if stride == 256 else 76))[0] for ii in range(n)]
