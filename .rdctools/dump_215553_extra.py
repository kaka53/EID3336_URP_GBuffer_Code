...
EIDS = [3761, 3765]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215553_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                inst_cb = (key == 'VS' and name == 'uniforms28') or (key == 'PS' and name == 'uniforms20')
                if inst_cb and sz >= 256:
                ...
                elif (key == 'PS' and name == 'uniforms28' and sz == 368) or (name == 'uniforms17' and sz == 3200) or (name == 'uniforms30' and sz == 16):
                    ...
                    if key == 'PS' and name == 'uniforms28':
                        ...
                        sl['vars'] = vars
                    elif name == 'uniforms17':
                        sl['mip416'] = floats(raw[416:420])[0]
                    elif name == 'uniforms30':
                        sl['floats'] = floats(raw)
    ...
    summary = {
        ...
        'u28nvars': [len(out[str(e)]['slices'].get('PS_uniforms28', {}).get('vars', [])) for e in EIDS],
        'u28size': [out[str(e)]['slices'].get('PS_uniforms28', {}).get('size') for e in EIDS],
        'u28floats': [out[str(e)]['slices'].get('PS_uniforms28', {}).get('floats') for e in EIDS],
        'mip': [out[str(e)]['slices'].get('PS_uniforms17', {}).get('mip416') for e in EIDS],
        'meta': [out[str(e)]['slices'].get('PS_uniforms30', {}).get('floats') for e in EIDS],
