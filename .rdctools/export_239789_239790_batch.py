...
EIDS = [1727]
FAMILY = 'VS239789_PS239790'
EXPECTED = (239789, 239790)
INSTANCE_STRIDE = 256
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS239789_PS239790_Batch'
...
EXPECTED_INPUTS = ['_input%d' % i for i in range(7)]
SSBO_SRC = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215441_PS215442_Batch/Captured/CBuffers/VS_00_ssbo27_8917ef5d197f9a03.bytes'
...
def work(c):
    ...
    ssbo_dst = os.path.join(CBS, 'VS_00_ssbo27_8917ef5d197f9a03.bytes')
    if not os.path.exists(ssbo_dst):
        shutil.copyfile(SSBO_SRC, ssbo_dst)
    ssbo_file = {
        'file': 'Captured/CBuffers/VS_00_ssbo27_8917ef5d197f9a03.bytes',
        'bytes': os.path.getsize(ssbo_dst),
        'sha256': '8917ef5d197f9a039b2b77fb133aa847accb7234e800627024f1fb72c608cc4d',
        'reused': True,
    }
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                if name in ('uniforms25', 'uniforms19') and sz >= INSTANCE_STRIDE:
                    need = instance_count * INSTANCE_STRIDE
                    raw = getbuf(d.resource, d.byteOffset, need)
                    h = hashlib.sha256(raw).hexdigest()
                    fr = save(os.path.join(INST, 'EID%d_%s_%d.bytes' % (eid, name, instance_count)), raw)
    ...
    shader_files = {
        'VS239789': {
            'spv': save(os.path.join(SH, 'VS239789.spv'), open(os.path.join(SHADER_SRC, 'c4dd8d3f9f3b1b5b_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS239789.spvasm'), open(os.path.join(SHADER_SRC, 'c4dd8d3f9f3b1b5b_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, 'c4dd8d3f9f3b1b5b_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS239790': {
            'spv': save(os.path.join(SH, 'PS239790.spv'), open(os.path.join(SHADER_SRC, '6a9a760b1ffcb649_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS239790.spvasm'), open(os.path.join(SHADER_SRC, '6a9a760b1ffcb649_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, '6a9a760b1ffcb649_PS.spv'), 'rb').read()).hexdigest(),
        },
    }

    existing = {}
    search_roots = [
        os.path.join(ASSETS, 'ColourPass3_VS209978_PS209979_Batch', 'TextureDatabase'),
        os.path.join(ASSETS, 'ColourPass6_VS215441_PS215442_Batch', 'TextureDatabase'),
        os.path.join(ASSETS, 'ColourPass6_VS215452_PS215453_Batch', 'TextureDatabase'),
        os.path.join(ASSETS, 'ColourPass6_VS239789_PS239790_Batch', 'TextureDatabase'),
    ]
    ...
    out = {
        ...
        'notes': {
            ...
            'localMaterialCB': 'PS uniforms24 304B packed _P00.._P18',
            'skinning': 'flags 52 bit32 set; ssbo27 RID261 reused from family 25 dump; no clip / no res27',
            'raster': raster,
            'liveUnityVP': True,
            ...
            'vsSha256': shader_files['VS239789']['sha256'],
            'psSha256': shader_files['PS239790']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS239789_PS239790_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_239789_239790_batch_result.json'
    summary = {
