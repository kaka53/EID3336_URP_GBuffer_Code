...
def work(c):
    ...
    shader_files = {
        'VS215511': {
            'spv': save(os.path.join(SH, 'VS215511.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS215511.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS215514': {
            'spv': save(os.path.join(SH, 'PS215514.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS215514.spvasm'), ps_asm),
    ...
    out = {
        ...
        'notes': {
            ...
            'vsSha256': shader_files['VS215511']['sha256'],
            'psSha256': shader_files['PS215514']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215511_PS215514_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
