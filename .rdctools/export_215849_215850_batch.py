...
EIDS = [3863, 3867, 3871, 3875, 3880]
FAMILY = 'VS215849_PS215850'
EXPECTED = (215849, 215850)
INSTANCE_STRIDE = 96
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215849_PS215850_Batch'
...
def work(c):
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if key == 'VS' and name == 'uniforms28':
                    ...
                    if len(raw) < need:
                        raise RuntimeError('EID%d uniforms28 too small %d < %d' % (eid, len(raw), need))
                    sliced = raw[:need]
                    instance_file = save(os.path.join(INST, 'EID%d_uniforms28_%d.bytes' % (eid, instance_count)), sliced)
            cbs[key] = arr
    ...
    shader_files = {}
    for stage, key in [(rd.ShaderStage.Vertex, 'VS215849'), (rd.ShaderStage.Pixel, 'PS215850')]:
        refl = s.GetShaderReflection(stage)
        shader_files[key] = {
    ...
    out = {
        ...
        'requestedEIDs': EIDS,
        'notes': {
            'vsSha256': shader_files['VS215849']['sha256'],
            'psSha256': shader_files['PS215850']['sha256'],
            'instanceStride': INSTANCE_STRIDE,
    ...
    open(os.path.join(ROOT, 'VS215849_PS215850_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215849_215850_batch_result.json'
    summary = {
