...
def work(c):
    ...
    shader_files = {
        'VS215527': {
            'spv': save(os.path.join(SH, 'VS215527.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS215527.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS215528': {
            'spv': save(os.path.join(SH, 'PS215528.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS215528.spvasm'), ps_asm),
    ...
    out = {
        ...
        'notes': {
            ...
            'vsSha256': shader_files['VS215527']['sha256'],
            'psSha256': shader_files['PS215528']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215527_PS215528_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
