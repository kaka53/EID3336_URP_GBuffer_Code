...
EIDS = [3448]
FAMILY = 'VS215519_PS215520'
EXPECTED = (215519, 215520)
INSTANCE_STRIDE = 256
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215519_PS215520_Batch'
...
def work(c):
    ...
    ps_spv = open(os.path.join(SHADER_SRC, '86e06de4d07f39b8_PS.spv'), 'rb').read()
    ps_asm = open(os.path.join(SHADER_SRC, '86e06de4d07f39b8_PS.spvasm'), 'rb').read()
    shader_files = {
        'VS215519': {
            'spv': save(os.path.join(SH, 'VS215519.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS215519.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS215520': {
            'spv': save(os.path.join(SH, 'PS215520.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS215520.spvasm'), ps_asm),
    ...
    out = {
        ...
        'notes': {
            ...
            'vsSha256': shader_files['VS215519']['sha256'],
            'psSha256': shader_files['PS215520']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215519_PS215520_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215519_215520_batch_result.json'
    summary = {
