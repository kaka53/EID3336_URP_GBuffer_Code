...
DST = Path(r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215519_PS215520_Batch/Shaders')
...
labels = {
    ...
    11: '基础颜色UV',
    12: '法线贴图UV',
    22: 'extra/fade 粗糙度源 遮罩选择 法线 混合',
    23: 'extra/fade 近远 与UV混合',
    24: 'extra/fade 颜色',
    25: 'extra/fade UV',
    26: 'extra maps 轴 混合偏置 UV缩放 法线',
    27: 'extra maps A/AO/RNM',
    28: 'extra maps 材质衰减与遮罩',
    29: 'extra maps 色倍率',
    30: 'extra maps 颜色乘色',
    31: 'extra maps UV偏移',
    32: 'extra maps 遮罩UV与采样源',
    33: 'overlay UV混合 层0粗糙度 层粗糙度lo',
    34: 'overlay 层粗糙度hi',
    35: 'overlay 材质Y',
    36: 'overlay lo/hi xy',
    37: 'overlay lo/hi z',
    38: 'overlay 层0 RGB与权重',
    41: 'overlay 细节UV',
    42: 'overlay 层1/层2 UV缩放',
    43: 'overlay 层1 RGB与权重',
    44: 'overlay 层2 RGB与权重',
}

props = []
for i in range(48):
    lab = labels.get(i, '捕获局部参数')
    props.append('        _P%02d ("c%02d %s", Vector) = (0,0,0,0)' % (i, i, lab))

shader = '''Shader "EID/URP/VS215519_PS215520_GBuffer"
{
    Properties
    {
...
(DST / 'EID215519215520GBuffer.shader').write_text(shader, encoding='utf8')

p_lines = []
for i in range(0, 48, 4):
    p_lines.append('float4 _P%02d; float4 _P%02d; float4 _P%02d; float4 _P%02d;' % (i, i+1, i+2, i+3))

hlsl = r'''#ifndef EID215519_215520_GBUFFER_INCLUDED
#define EID215519_215520_GBUFFER_INCLUDED
...
TEXTURE2D(_Res43); SAMPLER(sampler_Res43);
TEXTURE2D(_Res44); SAMPLER(sampler_Res44);

CBUFFER_START(UnityPerMaterial)
''' + '\n'.join(p_lines) + r'''
float4 _InstanceMeta;
float4 _InstanceStateYZ;
float _EID215520MipBias;
CBUFFER_END

struct Attributes215519
{
...
float3 DecodeOctNormal215519(uint packed)
{
...
float4 DecodePackedTangent215519(uint packed, float3 n)
{
...
float2 FilterNormalXY215519(float2 raw)
{
    float2 nxy = raw * 2.0 - 1.0;
    return float2(abs(nxy.x) < 0.012 ? 0.0 : nxy.x, abs(nxy.y) < 0.012 ? 0.0 : nxy.y);
}

Varyings215519 EID215519Vertex(Attributes215519 input)
{
    Varyings215519 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 normalOS = packedBasis ? DecodeOctNormal215519(packed) : input.packedNormal.xyz;
    float4 tangentOS = packedBasis ? DecodePackedTangent215519(packed, normalOS) : input.input2;
...
GBufferOutput215520 EID215520Fragment(Varyings215519 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215520 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float mip = _EID215520MipBias;
    ...
    float2 nxy = FilterNormalXY215519(normalSample.xy);
    ...
    float2 fxy = FilterNormalXY215519(extraFadeSample.xy) * (extraMaskFade * _P22.z);
    float3 fadeTS = float3(fxy, sqrt(saturate(1.0 - dot(FilterNormalXY215519(extraFadeSample.xy), FilterNormalXY215519(extraFadeSample.xy)))));
    ...
    float2 mixUV = lerp(input.uv0, input.uv1, _P33.x);
    float2 uvOverlay = mixUV * _P41.xy + _P41.zw;
    float3 detail = SAMPLE_TEXTURE2D_BIAS(_Res37, sampler_Res37, uvOverlay, mip).xyz;
    float4 layer1s = SAMPLE_TEXTURE2D_BIAS(_Res43, sampler_Res43, mixUV * _P42.x, mip);
    float4 layer2s = SAMPLE_TEXTURE2D_BIAS(_Res44, sampler_Res44, mixUV * _P42.y, mip);
    float3 lo = float3(_P36.x, _P36.z, _P37.x);
    float3 hi = float3(_P36.y, _P36.w, _P37.y);
    float3 weights = saturate(lo * 0.5 + lerp(-lo, 1.0, detail + hi)) * float3(_P38.w, _P43.w, _P44.w);
    float2 layerRough = lerp(float2(_P33.z, _P34.y), float2(_P33.w, _P34.z), float2(layer1s.w, layer2s.w));
    float4 layered = float4(baseColor, roughness);
    layered = lerp(layered, float4(layer2s.xyz * _P44.xyz, layerRough.y), weights.zzz);
    layered = lerp(layered, float4(layer1s.xyz * _P43.xyz, layerRough.x), weights.yyy);
    layered = lerp(layered, float4(_P38.xyz, _P33.y), weights.xxx);
...
(DST / 'EID215519215520GBuffer.hlsl').write_text(hlsl, encoding='utf8')
print('shader', (DST / 'EID215519215520GBuffer.shader').stat().st_size)
print('hlsl', (DST / 'EID215519215520GBuffer.hlsl').stat().st_size)
print('P decls', len(p_lines))
