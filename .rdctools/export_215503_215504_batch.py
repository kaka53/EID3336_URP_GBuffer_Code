...
EIDS = [3246, 3250, 3255]
FAMILY = 'VS215503_PS215504'
EXPECTED = (215503, 215504)
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215503_PS215504_Batch'
...
UNIQUE_MATERIAL = {'res24', 'res26'}
SHARED_REQUIRED = set()
ASSETS = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets'
EXPECTED_INPUTS = {'_input0', '_input1', '_input2', '_input3', '_input4', '_input5', '_input6', '_input8', '_input9'}
...
def work(c):
    ...
    for eid in EIDS:
        ...
        rec_raster = {
            ...
            'stencilRef': int(st[0].reference) if st else 0,
        ...
        if names != EXPECTED_INPUTS:
            raise RuntimeError('EID%d expected VS inputs %s, got %s' % (eid, sorted(EXPECTED_INPUTS), sorted(names)))
    ...
    shader_files = {}
    for stage, key in [(rd.ShaderStage.Vertex, 'VS215503'), (rd.ShaderStage.Pixel, 'PS215504')]:
        refl = s.GetShaderReflection(stage)
        shader_files[key] = {
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms30 / PS uniforms21 stride 256',
            'localMaterialCB': 'PS uniforms29 448B / VS uniforms36 same bytes',
            'materialIdCB': 'PS uniforms31 16B child3',
            'mipBias': 'PS uniforms18 child16 @416',
            'skinning': 'flags bit32 all zero; ssbo38 not baked',
            'raster': raster,
            'liveUnityVP': True,
    ...
    open(os.path.join(ROOT, 'VS215503_PS215504_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215503_215504_batch_result.json'
    summary = {
