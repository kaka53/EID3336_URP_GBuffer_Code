...
EIDS = [3583]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_243940.json'
        ...
        for m in members:
            ...
            if members2:
                ...
                out.append({
                    ...
                    'rows': rows,
                    'cols': cols,
                    'array': [int(x) for x in list(getattr(t, 'array', []) or [])],
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if inst_cb and sz >= 256:
                    ...
                    if key == 'VS':
                        ...
                        rec['mats'] = [floats(raw[ii * 256:ii * 256 + 16]) + floats(raw[ii * 256 + 48:ii * 256 + 64]) for ii in range(n)]
                        rec['instHead16'] = floats(raw[:64])
                elif dump_full and not inst_cb:
                    ...
                    rec['slices']['%s_%s' % (key, name)] = {'size': sz, 'rid': rid(d.resource), 'skipped': True}
