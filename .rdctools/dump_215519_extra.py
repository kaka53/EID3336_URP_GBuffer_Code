...
EIDS = [3448]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215519_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                inst_cb = (key == 'VS' and name == 'uniforms30') or (key == 'PS' and name == 'uniforms24')
                local_cb = name in ('uniforms46', 'uniforms48', 'uniforms50')
                mip = (name in ('uniforms27', 'uniforms21') and sz == 3200)
    ...
    summary = {
        ...
        'blend0': [out[str(e)]['blend0'] for e in EIDS],
        ...
        'u46sha': [out[str(e)]['slices'].get('PS_uniforms46', {}).get('sha16') for e in EIDS],
        'u46nvars': [out[str(e)]['slices'].get('PS_uniforms46', {}).get('nvars') for e in EIDS],
        'u46size': [out[str(e)]['slices'].get('PS_uniforms46', {}).get('size') for e in EIDS],
        'u46floats': [out[str(e)]['slices'].get('PS_uniforms46', {}).get('floats') for e in EIDS],
        'u48': [out[str(e)]['slices'].get('PS_uniforms48') for e in EIDS],
        'u50sha': [out[str(e)]['slices'].get('PS_uniforms50', {}).get('sha16') for e in EIDS],
        'u50size': [out[str(e)]['slices'].get('PS_uniforms50', {}).get('size') for e in EIDS],
        'u50floats': [out[str(e)]['slices'].get('PS_uniforms50', {}).get('floats') for e in EIDS],
        'mipVS': [out[str(e)]['slices'].get('VS_uniforms27_mip416') for e in EIDS],
        'mipPS': [out[str(e)]['slices'].get('PS_uniforms21_mip416') for e in EIDS],
        'globalY': [out[str(e)]['slices'].get('PS_uniforms21_y436') for e in EIDS],
