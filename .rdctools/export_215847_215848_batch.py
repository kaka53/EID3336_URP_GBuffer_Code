...
EIDS = [3858]
FAMILY = 'VS215847_PS215848'
EXPECTED = (215847, 215848)
INSTANCE_STRIDE = 96
EXPECTED_INPUTS = ['_input0', '_input1', '_input2', '_input3', '_input4', '_input5', '_input6']
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215847_PS215848_Batch'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        if sorted(names) != sorted(EXPECTED_INPUTS):
            raise RuntimeError('EID%d unexpected inputs %s' % (eid, names))
        vbs = s.GetVBuffers()
        layouts = []
        streams = []
        stream_blobs = []
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if key == 'VS' and name == 'uniforms27':
                    ...
                    instance_file = save(os.path.join(INST, 'EID%d_uniforms27_%d.bytes' % (eid, instance_count)), raw)
        ...
        if instance_file is None:
            raise RuntimeError('EID%d missing VS uniforms27 instance array' % eid)
    ...
    shader_files = {
        'VS215847': {
            'spv': save(os.path.join(SH, 'VS215847.spv'), open(os.path.join(shader_src, '1e875fd6a16f85f9_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS215847.spvasm'), open(os.path.join(shader_src, '1e875fd6a16f85f9_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, '1e875fd6a16f85f9_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS215848': {
            'spv': save(os.path.join(SH, 'PS215848.spv'), open(os.path.join(shader_src, '3a45435d154b42d3_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS215848.spvasm'), open(os.path.join(shader_src, '3a45435d154b42d3_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, '3a45435d154b42d3_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    search_roots = [
        ...
        os.path.join(ASSETS, 'ColourPass6_VS215847_PS215848_Batch', 'TextureDatabase'),
    ]
    ...
    out = {
        ...
        'requestedEIDs': EIDS,
        'notes': {
            'vsSha256': shader_files['VS215847']['sha256'],
            'psSha256': shader_files['PS215848']['sha256'],
            'instanceStride': INSTANCE_STRIDE,
    ...
    open(os.path.join(ROOT, 'VS215847_PS215848_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215847_215848_batch_result.json'
    summary = {
