...
UNIQUE_MATERIAL = {'res34', 'res36', 'res38'}
EXTRA_REUSE = {'res40', 'res41', 'res42'}
KEEP_TEXTURES = UNIQUE_MATERIAL | EXTRA_REUSE
SKIP_CB = {'uniforms48', 'uniforms50', 'uniforms52'}
...
def is_instance_cb(key, name, sz):
    if sz < INSTANCE_STRIDE:
        return False
    return (key == 'VS' and name == 'uniforms30') or (key == 'PS' and name == 'uniforms25')
...
def work(c):
    ...
    vs_spv = open(os.path.join(SHADER_SRC, '8d02447e001351fe_VS.spv'), 'rb').read()
    vs_asm = open(os.path.join(SHADER_SRC, '8d02447e001351fe_VS.spvasm'), 'rb').read()
    ...
    out = {
        ...
        'notes': {
            ...
            'psReuseSlots': sorted(EXTRA_REUSE),
            'instanceBuffer': 'VS uniforms30 / PS uniforms25 stride 256 sliced by size',
            'localMaterialCB': 'PS uniforms44 512B packed _P00.._P31',
            'materialIdCB': 'PS uniforms46 16B',
            'mipBias': 'PS uniforms22 child16 @416',
            'skinning': 'flags bit32 all zero; skip 8.4MB ssbo32',
            'raster': raster,
            'liveUnityVP': True,
