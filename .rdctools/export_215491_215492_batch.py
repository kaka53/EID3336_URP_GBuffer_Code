...
def work(c):
    ...
    shader_files = {
        'VS215491': {
            'spv': save(os.path.join(SH, 'VS215491.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS215491.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS215492': {
            'spv': save(os.path.join(SH, 'PS215492.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS215492.spvasm'), ps_asm),
    ...
    out = {
        ...
        'notes': {
            ...
            'vsSha256': shader_files['VS215491']['sha256'],
            'psSha256': shader_files['PS215492']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215491_PS215492_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
