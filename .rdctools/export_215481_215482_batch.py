...
EIDS = [1766, 1770]
FAMILY = 'VS215481_PS215482'
EXPECTED = (215481, 215482)
INSTANCE_STRIDE = 256
...
UNIQUE_MATERIAL = {'res12'}
ASSETS = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets'
EXPECTED_INPUTS = ['_input%d' % i for i in range(8)]
SSBO_SRC = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215479_PS215480_Batch/Captured/CBuffers/VS_00_ssbo31_8917ef5d197f9a03.bytes'
...
def work(c):
    ...
    ssbo_dst = os.path.join(CBS, 'VS_00_ssbo31_8917ef5d197f9a03.bytes')
    if not os.path.exists(ssbo_dst):
        shutil.copyfile(SSBO_SRC, ssbo_dst)
    ssbo_file = {
        'file': 'Captured/CBuffers/VS_00_ssbo31_8917ef5d197f9a03.bytes',
    ...
    for eid in EIDS:
        ...
        if sorted(names) != sorted(EXPECTED_INPUTS):
            raise RuntimeError('EID%d expected 8 VS inputs _input0.._input7, got %s' % (eid, names))
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                if name == 'uniforms26' and sz >= INSTANCE_STRIDE:
    ...
    shader_files = {
        'VS215481': {
            'spv': save(os.path.join(SH, 'VS215481.spv'), open(os.path.join(SHADER_SRC, 'e297216d76cb3720_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS215481.spvasm'), open(os.path.join(SHADER_SRC, 'e297216d76cb3720_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, 'e297216d76cb3720_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS215482': {
            'spv': save(os.path.join(SH, 'PS215482.spv'), open(os.path.join(SHADER_SRC, '5c2b1e0c0e0738ad_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS215482.spvasm'), open(os.path.join(SHADER_SRC, '5c2b1e0c0e0738ad_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, '5c2b1e0c0e0738ad_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    search_roots = [
        ...
        os.path.join(ASSETS, 'ColourPass6_VS215481_PS215482_Batch', 'TextureDatabase'),
    ]
    ...
    out = {
        ...
        'notes': {
            ...
            'localMaterialCB': 'PS uniforms11 352B packed _P00.._P21',
            'skinning': 'flags 52 bit32 set; ssbo31 RID261 reused from family 28 dump',
            'raster': raster,
            'liveUnityVP': True,
            ...
            'vsSha256': shader_files['VS215481']['sha256'],
            'psSha256': shader_files['PS215482']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215481_PS215482_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215481_215482_batch_result.json'
    summary = {
