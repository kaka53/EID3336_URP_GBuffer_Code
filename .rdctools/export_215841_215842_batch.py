...
EIDS = [3781, 3784, 3787, 3790, 3793, 3796, 3799, 3802, 3805, 3809, 3812, 3816, 3819, 3822, 3825, 3828, 3831, 3835, 3839]
FAMILY = 'VS215841_PS215842'
EXPECTED = (215841, 215842)
INSTANCE_STRIDE = 96
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215841_PS215842_Batch'
...
def work(c):
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if key == 'VS' and name == 'uniforms27':
                    ...
                    if len(raw) < need:
                        raise RuntimeError('EID%d uniforms27 too small %d < %d' % (eid, len(raw), need))
                    sliced = raw[:need]
                    instance_file = save(os.path.join(INST, 'EID%d_uniforms27_%d.bytes' % (eid, instance_count)), sliced)
            cbs[key] = arr
    ...
    shader_files = {}
    for stage, key in [(rd.ShaderStage.Vertex, 'VS215841'), (rd.ShaderStage.Pixel, 'PS215842')]:
        refl = s.GetShaderReflection(stage)
        shader_files[key] = {
    ...
    out = {
        ...
        'requestedEIDs': EIDS,
        'notes': {
            'vsSha256': shader_files['VS215841']['sha256'],
            'psSha256': shader_files['PS215842']['sha256'],
            'instanceStride': INSTANCE_STRIDE,
    ...
    open(os.path.join(ROOT, 'VS215841_PS215842_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215841_215842_batch_result.json'
    summary = {
