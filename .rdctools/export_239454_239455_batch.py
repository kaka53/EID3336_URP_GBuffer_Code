...
EIDS = [3573, 3577]
FAMILY = 'VS239454_PS239455'
EXPECTED = (239454, 239455)
INSTANCE_STRIDE = 256
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS239454_PS239455_Batch'
...
KEEP_TEXTURES = set(UNIQUE_MATERIAL)
SKIP_CB = set()
...
def work(c):
    ...
    for eid in EIDS:
        ...
        if sorted(names) != sorted(EXPECTED_INPUTS):
            raise RuntimeError('EID%d expected 8 VS inputs _input0..5,_input7,_input8, got %s' % (eid, names))
        for slot in sorted(set(x.vertexBuffer for x in inputs)):
            ...
            if stride > 0:
                ...
                size = max(int(x.byteOffset) + max(4, int(x.format.compCount) * int(x.format.compByteWidth)) for x in sin)
                count = 1
                start = 0
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                elif name in SKIP_CB or sz > 4096:
                    raw = b''
    ...
    vs_spv = open(os.path.join(SHADER_SRC, 'e5796398cae4dfb5_VS.spv'), 'rb').read()
    vs_asm = open(os.path.join(SHADER_SRC, 'e5796398cae4dfb5_VS.spvasm'), 'rb').read()
    ps_spv = open(os.path.join(SHADER_SRC, 'b0feed875ab59322_PS.spv'), 'rb').read()
    ps_asm = open(os.path.join(SHADER_SRC, 'b0feed875ab59322_PS.spvasm'), 'rb').read()
    shader_files = {
        'VS239454': {
            'spv': save(os.path.join(SH, 'VS239454.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS239454.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS239455': {
            'spv': save(os.path.join(SH, 'PS239455.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS239455.spvasm'), ps_asm),
    ...
    existing = {}
    search_roots = [
        os.path.join(ASSETS, 'ColourPass6_VS239454_PS239455_Batch', 'TextureDatabase'),
        os.path.join(ASSETS, 'ColourPass6_VS230642_PS230643_Batch', 'TextureDatabase'),
    ...
    for r, v in sorted(texture_refs.items()):
        ...
        if os.path.exists(path):
            ...
            continue
        pending.append({'rid': r, 'path': path, 'rel': rel})
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms27 / PS uniforms20 stride 256 sliced by instanceCount',
            'localMaterialCB': 'PS uniforms32 / VS uniforms33 304B packed _P00.._P18',
            'mipBias': 'PS uniforms17 child16 @416',
            'skinning': 'flags bit32 all zero; skip 8.4MB ssbo35',
            'raster': raster,
            'liveUnityVP': True,
            'packedNormal': 'NORMAL.x from _input1 oct 0.0020',
            'vat': 'dual-frame blend input5.x/y/z; rows child9.z/w',
            ...
            'vsSha256': shader_files['VS239454']['sha256'],
            'psSha256': shader_files['PS239455']['sha256'],
    ...
    for r, v in sorted(texture_refs.items()):
        ...
        keep = bool(KEEP_TEXTURES.intersection(v['bindings']))
        ...
        elif keep and v['existingAssets']:
    ...
    open(os.path.join(ROOT, 'VS239454_PS239455_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_239454_239455_batch_result.json'
    summary = {
        ...
        'profileTextures': [{p['eid']: [t['name'] for t in p['textures'] if t['name'] in KEEP_TEXTURES]} for p in profiles],
        'translations': [[m[0][3], m[1][3], m[2][3]] for p in profiles for m in p['instanceMatricesRowMajor']],
    }
