...
EIDS = [3669, 3674]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215539_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                if ((key == 'VS' and name == 'uniforms27') or (key == 'PS' and name == 'uniforms23')) and sz >= 256:
                ...
                elif name in ('uniforms33', 'uniforms35') or (name == 'uniforms20' and sz == 3200):
    ...
    summary = {
        ...
        'u33nvars': [len(out[str(e)]['slices'].get('PS_uniforms33', {}).get('vars', [])) for e in EIDS],
        'u33size': [out[str(e)]['slices'].get('PS_uniforms33', {}).get('size') for e in EIDS],
        'u35': [out[str(e)]['slices'].get('PS_uniforms35', {}).get('floats') for e in EIDS],
        'mip': [out[str(e)]['slices'].get('PS_uniforms20', {}).get('mip416') for e in EIDS],
