...
EIDS = [3885, 3889, 3895]
FAMILY = 'VS215851_PS215852'
EXPECTED = (215851, 215852)
INSTANCE_STRIDE = 96
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215851_PS215852_Batch'
...
VS_SHARED = {'res34','res35','res36','res37'}
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                if key == 'VS' and name == 'uniforms28':
                    ...
                    instance_file = save(os.path.join(INST, 'EID%d_uniforms28_%d.bytes' % (eid, instance_count)), raw)
        ...
        if instance_file is None:
            raise RuntimeError('EID%d missing VS uniforms28 instance array' % eid)
    ...
    shader_files = {
        'VS215851': {
            'spv': save(os.path.join(SH, 'VS215851.spv'), open(os.path.join(shader_src, '8fca51be7d8f9ad5_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS215851.spvasm'), open(os.path.join(shader_src, '8fca51be7d8f9ad5_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, '8fca51be7d8f9ad5_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS215852': {
            'spv': save(os.path.join(SH, 'PS215852.spv'), open(os.path.join(shader_src, '8656f77f9e6539bf_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS215852.spvasm'), open(os.path.join(shader_src, '8656f77f9e6539bf_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, '8656f77f9e6539bf_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    out = {
        ...
        'requestedEIDs': EIDS,
        'notes': {
            'vsSha256': shader_files['VS215851']['sha256'],
            'psSha256': shader_files['PS215852']['sha256'],
            'instanceStride': INSTANCE_STRIDE,
    ...
    open(os.path.join(ROOT, 'VS215851_PS215852_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215851_215852_batch_result.json'
    summary = {
