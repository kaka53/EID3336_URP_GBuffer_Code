...
EIDS = [3231]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215491_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                inst_cb = (key == 'VS' and name == 'uniforms28') or (key == 'PS' and name == 'uniforms23')
                local_cb = (name == 'uniforms33' and sz == 416) or (name == 'uniforms35' and sz == 16)
                extra_small = (sz == 16 and name != 'uniforms35')
                mip = (name in ('uniforms25', 'uniforms20') and sz == 3200)
                cam = (name in ('uniforms23', 'uniforms18') and sz == 1312)
    ...
    summary = {
        ...
        'u33sha': [out[str(e)]['slices'].get('PS_uniforms33', {}).get('sha16') for e in EIDS],
        'u33nvars': [out[str(e)]['slices'].get('PS_uniforms33', {}).get('nvars') for e in EIDS],
        'u33floats': [out[str(e)]['slices'].get('PS_uniforms33', {}).get('floats') for e in EIDS],
        'u35': [out[str(e)]['slices'].get('PS_uniforms35') for e in EIDS],
        'mipVS': [out[str(e)]['slices'].get('VS_uniforms25_mip416') for e in EIDS],
        'mipPS': [out[str(e)]['slices'].get('PS_uniforms20_mip416') for e in EIDS],
        'globalY': [out[str(e)]['slices'].get('PS_uniforms20_y436') for e in EIDS],
        'cam': [out[str(e)]['slices'].get('VS_uniforms23_child11') for e in EIDS],
        ...
        'instSlice': [out[str(e)]['slices'].get('VS_uniforms28') for e in EIDS],
