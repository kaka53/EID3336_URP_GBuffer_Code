...
EIDS = [3237, 3241]
FAMILY = 'VS215496_PS215498'
EXPECTED = (215496, 215498)
INSTANCE_STRIDE = 256
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215496_PS215498_Batch'
...
UNIQUE_MATERIAL = {'res24', 'res26', 'res32'}
...
def is_instance_cb(key, name, sz):
    if sz < INSTANCE_STRIDE:
        return False
    return (key == 'VS' and name == 'uniforms28') or (key == 'PS' and name == 'uniforms21')
...
def work(c):
    ...
    vs_spv = open(os.path.join(SHADER_SRC, 'ff0072ca14416e5d_VS.spv'), 'rb').read()
    vs_asm = open(os.path.join(SHADER_SRC, 'ff0072ca14416e5d_VS.spvasm'), 'rb').read()
    ps_spv = open(os.path.join(SHADER_SRC, '17cb06f106d07bcc_PS.spv'), 'rb').read()
    ps_asm = open(os.path.join(SHADER_SRC, '17cb06f106d07bcc_PS.spvasm'), 'rb').read()
    shader_files = {
        'VS215496': {
            'spv': save(os.path.join(SH, 'VS215496.spv'), vs_spv),
            'disassembly': save(os.path.join(SH, 'VS215496.spvasm'), vs_asm),
            'sha256': hashlib.sha256(vs_spv).hexdigest(),
        },
        'PS215498': {
            'spv': save(os.path.join(SH, 'PS215498.spv'), ps_spv),
            'disassembly': save(os.path.join(SH, 'PS215498.spvasm'), ps_asm),
    ...
    existing = {}
    search_roots = [
        os.path.join(ASSETS, 'ColourPass6_VS215496_PS215498_Batch', 'TextureDatabase'),
        os.path.join(ASSETS, 'ColourPass6_VS215503_PS215504_Batch', 'TextureDatabase'),
        os.path.join(ASSETS, 'ColourPass6_VS215541_PS215542_Batch', 'TextureDatabase'),
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms28 / PS uniforms21 stride 256 sliced by size',
            'localMaterialCB': 'PS uniforms29 / VS uniforms34 448B packed _P00.._P27',
            'materialIdCB': 'PS uniforms31 16B',
            'mipBias': 'PS uniforms18 child16 @416',
            'windGlobals': 'VS uniforms25 @512/@880/@896',
            ...
            'vsSha256': shader_files['VS215496']['sha256'],
            'psSha256': shader_files['PS215498']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215496_PS215498_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215496_215498_batch_result.json'
    summary = {
