...
EIDS = [3573, 3577]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_239454_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                inst_cb = (key == 'VS' and name == 'uniforms27') or (key == 'PS' and name == 'uniforms20')
                local_cb = (name in ('uniforms32', 'uniforms33') and sz == 304)
                extra_small = (sz == 16)
                mip = (name in ('uniforms24', 'uniforms17') and sz == 3200)
                cam = (name in ('uniforms22',) and sz == 1312)
    ...
    summary = {
        ...
        'u32sha': [out[str(e)]['slices'].get('PS_uniforms32', {}).get('sha16') for e in EIDS],
        'u32nvars': [out[str(e)]['slices'].get('PS_uniforms32', {}).get('nvars') for e in EIDS],
        'u32floats': [out[str(e)]['slices'].get('PS_uniforms32', {}).get('floats') for e in EIDS],
        'u33sha': [out[str(e)]['slices'].get('VS_uniforms33', {}).get('sha16') for e in EIDS],
        'u33floats': [out[str(e)]['slices'].get('VS_uniforms33', {}).get('floats') for e in EIDS],
        'mipVS': [out[str(e)]['slices'].get('VS_uniforms24_mip416') for e in EIDS],
        'mipPS': [out[str(e)]['slices'].get('PS_uniforms17_mip416') for e in EIDS],
        'globalY': [out[str(e)]['slices'].get('PS_uniforms17_y436') for e in EIDS],
