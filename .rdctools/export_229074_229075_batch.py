...
EIDS = [3739, 3743, 3749]
FAMILY = 'VS229074_PS229075'
EXPECTED = (229074, 229075)
INSTANCE_STRIDE = 256
...
UNIQUE_MATERIAL = {'res23', 'res25', 'res27', 'res29'}
ASSETS = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets'
EXPECTED_INPUTS = ['_input%d' % i for i in range(9)]
SSBO30_SRC = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215505_PS215507_Batch/Captured/CBuffers/VS_00_ssbo30_8917ef5d197f9a03.bytes'
...
def work(c):
    ...
    ssbo_dst = os.path.join(CBS, 'VS_00_ssbo30_8917ef5d197f9a03.bytes')
    if not os.path.exists(ssbo_dst):
        shutil.copyfile(SSBO30_SRC, ssbo_dst)
    ssbo_file = {
        'file': 'Captured/CBuffers/VS_00_ssbo30_8917ef5d197f9a03.bytes',
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                if name in ('uniforms28', 'uniforms20') and sz >= INSTANCE_STRIDE:
    ...
    shader_files = {
        'VS229074': {
            'spv': save(os.path.join(SH, 'VS229074.spv'), open(os.path.join(SHADER_SRC, '80eeee493f5434da_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS229074.spvasm'), open(os.path.join(SHADER_SRC, '80eeee493f5434da_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, '80eeee493f5434da_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS229075': {
            'spv': save(os.path.join(SH, 'PS229075.spv'), open(os.path.join(SHADER_SRC, 'd747a5ff22d9f172_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS229075.spvasm'), open(os.path.join(SHADER_SRC, 'd747a5ff22d9f172_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, 'd747a5ff22d9f172_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    search_roots = [
        ...
        os.path.join(ASSETS, 'ColourPass6_VS229074_PS229075_Batch', 'TextureDatabase'),
    ]
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms28 / PS uniforms20 stride 256',
            'localMaterialCB': 'PS uniforms32 512B packed _P00.._P31',
            'skinning': 'flags 49/52 bit32 set; ssbo30 RID261 reused from family 12 dump',
            'raster': raster,
            'liveUnityVP': True,
            ...
            'vsSha256': shader_files['VS229074']['sha256'],
            'psSha256': shader_files['PS229075']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS229074_PS229075_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_229074_229075_batch_result.json'
    summary = {
