...
EIDS = [3617, 3621]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215537_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                if ((key == 'VS' and name == 'uniforms29') or (key == 'PS' and name == 'uniforms25')) and sz >= 256:
                ...
                elif name in ('uniforms38', 'uniforms40') or (name == 'uniforms22' and sz == 3200):
    ...
    summary = {
        ...
        'u38sha': [out[str(e)]['slices'].get('PS_uniforms38', {}).get('sha16') for e in EIDS],
        'u38nvars': [len(out[str(e)]['slices'].get('PS_uniforms38', {}).get('vars', [])) for e in EIDS],
        'u38size': [out[str(e)]['slices'].get('PS_uniforms38', {}).get('size') for e in EIDS],
        'u40': [out[str(e)]['slices'].get('PS_uniforms40', {}).get('floats') for e in EIDS],
        'mip': [out[str(e)]['slices'].get('PS_uniforms22', {}).get('mip416') for e in EIDS],
