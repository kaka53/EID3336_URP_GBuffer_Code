...
EIDS = [1717, 1721]
FAMILY = 'VS215452_PS215453'
EXPECTED = (215452, 215453)
INSTANCE_STRIDE = 256
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215452_PS215453_Batch'
...
UNIQUE_MATERIAL = {'res25', 'res26', 'res27'}
...
def work(c):
    ...
    shader_files = {
        'VS215452': {
            'spv': save(os.path.join(SH, 'VS215452.spv'), open(os.path.join(SHADER_SRC, 'c4dd8d3f9f3b1b5b_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS215452.spvasm'), open(os.path.join(SHADER_SRC, 'c4dd8d3f9f3b1b5b_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, 'c4dd8d3f9f3b1b5b_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS215453': {
            'spv': save(os.path.join(SH, 'PS215453.spv'), open(os.path.join(SHADER_SRC, '7379b741524f60a0_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS215453.spvasm'), open(os.path.join(SHADER_SRC, '7379b741524f60a0_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(SHADER_SRC, '7379b741524f60a0_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    out = {
        ...
        'notes': {
            ...
            'skinning': 'flags 52 bit32 set; ssbo27 RID261 reused from family 25 dump',
            'raster': raster,
            'liveUnityVP': True,
            ...
            'vsSha256': shader_files['VS215452']['sha256'],
            'psSha256': shader_files['PS215453']['sha256'],
        },
        'profiles': profiles,
    ...
    open(os.path.join(ROOT, 'VS215452_PS215453_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215452_215453_batch_result.json'
    summary = {
