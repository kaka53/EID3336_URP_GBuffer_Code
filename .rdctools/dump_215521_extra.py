...
EIDS = [3536, 3540]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215521_extra.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                if ((key == 'VS' and name == 'uniforms30') or (key == 'PS' and name == 'uniforms24')) and sz >= 256:
                ...
                elif name in ('uniforms37', 'uniforms39') or (name == 'uniforms21' and sz == 3200):
    ...
    summary = {
        ...
        'u37sha': [out[str(e)]['slices'].get('PS_uniforms37', {}).get('sha16') for e in EIDS],
        'u37nvars': [len(out[str(e)]['slices'].get('PS_uniforms37', {}).get('vars', [])) for e in EIDS],
        'u37size': [out[str(e)]['slices'].get('PS_uniforms37', {}).get('size') for e in EIDS],
        'u39': [out[str(e)]['slices'].get('PS_uniforms39', {}).get('floats') for e in EIDS],
        'mip': [out[str(e)]['slices'].get('PS_uniforms21', {}).get('mip416') for e in EIDS],
        'pstex': [[(t['n'], t['rid'], t['fmt'], t['w']) for t in out[str(e)]['pstex']] for e in EIDS],
    }
    return summary
