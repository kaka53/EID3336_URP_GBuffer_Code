...
EIDS = [3453, 3458, 3462, 3467, 3472, 3476, 3480, 3485, 3490, 3494, 3498, 3502, 3506, 3510, 3514, 3518, 3522]
FAMILY = 'VS209988_PS209989'
EXPECTED = (209988, 209989)
...
UNIQUE_MATERIAL = {'res35', 'res37', 'res38'}
SHARED_REQUIRED = {'res33'}
...
def work(c):
    ...
    for eid in EIDS:
        ...
        if names != ['_input%d' % i for i in range(10)]:
    ...
    shader_files = {}
    for stage, key in [(rd.ShaderStage.Vertex, 'VS209988'), (rd.ShaderStage.Pixel, 'PS209989')]:
        refl = s.GetShaderReflection(stage)
        shader_files[key] = {
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms30 / PS uniforms24 stride 256',
            'localMaterialCB': 'PS uniforms43 720B',
            'skinning': 'flags bit32 all zero; ssbo32 not baked',
            'raster': raster,
            'liveUnityVP': True,
    ...
    open(os.path.join(ROOT, 'VS209988_PS209989_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_209988_209989_batch_result.json'
    summary = {
