...
EIDS = [3729]
OUT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/dump_215547.json'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                rec['cbs'].append({'stage': key, 'i': i, 'n': name, 'sz': sz, 'rid': rid(d.resource), 'off': int(d.byteOffset)})
                inst_cb = (key == 'VS' and name == 'uniforms30') or (key == 'PS' and name == 'uniforms25')
                local_cb = (key == 'PS' and name == 'uniforms44' and sz in (496, 512))
                extra_small = (key == 'PS' and name in ('uniforms48', 'uniforms46') and sz <= 256)
                mip = (name in ('uniforms27', 'uniforms22') and sz == 3200)
                cam = (name in ('uniforms25', 'uniforms20') and sz == 1312)
    ...
    summary = {
        ...
        'cbs': [out[str(e)]['cbs'] for e in EIDS],
        'u44sha': [out[str(e)]['slices'].get('PS_uniforms44', {}).get('sha16') for e in EIDS],
        'u44nvars': [out[str(e)]['slices'].get('PS_uniforms44', {}).get('nvars') for e in EIDS],
        'u44size': [out[str(e)]['slices'].get('PS_uniforms44', {}).get('size') for e in EIDS],
        'u44floats': [out[str(e)]['slices'].get('PS_uniforms44', {}).get('floats') for e in EIDS],
        'u44vars': [out[str(e)]['slices'].get('PS_uniforms44', {}).get('vars') for e in EIDS],
        'u48': [out[str(e)]['slices'].get('PS_uniforms48', {}).get('floats') for e in EIDS],
        'u46': [out[str(e)]['slices'].get('PS_uniforms46', {}).get('floats') for e in EIDS],
        'mipVS': [out[str(e)]['slices'].get('VS_uniforms27_mip416') for e in EIDS],
        'mipPS': [out[str(e)]['slices'].get('PS_uniforms22_mip416') for e in EIDS],
        'cam': [out[str(e)]['slices'].get('VS_uniforms25_child11') for e in EIDS],
        'pstex': [[(t['n'], t['rid'], t['fmt'], t['w'], t.get('bind'), t.get('type'), t.get('arraysize')) for t in out[str(e)]['pstex']] for e in EIDS],
        'vstex': [[(t['n'], t['rid'], t['fmt'], t['w'], t.get('bind')) for t in out[str(e)]['vstex']] for e in EIDS],
        'instSlice': [out[str(e)]['slices'].get('VS_uniforms30') for e in EIDS],
