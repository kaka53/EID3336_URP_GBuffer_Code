...
EIDS = [3849, 3853]
FAMILY = 'VS215845_PS215846'
EXPECTED = (215845, 215846)
INSTANCE_STRIDE = 96
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS215845_PS215846_Batch'
...
def work(c):
    ...
    shader_files = {
        'VS215845': {
            'spv': save(os.path.join(SH, 'VS215845.spv'), open(os.path.join(shader_src, 'd89f2edd5b0e7424_VS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'VS215845.spvasm'), open(os.path.join(shader_src, 'd89f2edd5b0e7424_VS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, 'd89f2edd5b0e7424_VS.spv'), 'rb').read()).hexdigest(),
        },
        'PS215846': {
            'spv': save(os.path.join(SH, 'PS215846.spv'), open(os.path.join(shader_src, 'df3dd1dcf42e17e1_PS.spv'), 'rb').read()),
            'disassembly': save(os.path.join(SH, 'PS215846.spvasm'), open(os.path.join(shader_src, 'df3dd1dcf42e17e1_PS.spvasm'), 'rb').read()),
            'sha256': hashlib.sha256(open(os.path.join(shader_src, 'df3dd1dcf42e17e1_PS.spv'), 'rb').read()).hexdigest(),
        },
    }
    ...
    out = {
        ...
        'requestedEIDs': EIDS,
        'notes': {
            'vsSha256': shader_files['VS215845']['sha256'],
            'psSha256': shader_files['PS215846']['sha256'],
            'instanceStride': INSTANCE_STRIDE,
    ...
    open(os.path.join(ROOT, 'VS215845_PS215846_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_215845_215846_batch_result.json'
    summary = {
