...
EIDS = [3755]
FAMILY = 'VS229351_PS229352'
EXPECTED = (229351, 229352)
ROOT = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/Assets/ColourPass6_VS229351_PS229352_Batch'
...
def work(c):
    ...
    for eid in EIDS:
        ...
        if raster is None:
            ...
            raster = {
                ...
                'stencilFrontRef': int(st[0].reference) if st else None,
                'stencilFrontFunction': str(st[0].function) if st else '',
        ...
        names = sorted(x.name for x in inputs)
        if names != sorted(EXPECTED_INPUTS):
            raise RuntimeError('EID%d expected 7 VS inputs _input0.._input6, got %s' % (eid, [x.name for x in inputs]))
        ...
        for stage, key in [(rd.ShaderStage.Vertex, 'VS'), (rd.ShaderStage.Pixel, 'PS')]:
            ...
            for i, u in enumerate(s.GetConstantBlocks(stage)):
                ...
                sz = int(d.byteSize)
                inst_cb = (key == 'VS' and name == 'uniforms27')
                if inst_cb:
                    take = int(a.numInstances) * INSTANCE_STRIDE
                    raw = getbuf(d.resource, d.byteOffset, take)
                elif sz > 4096:
                    skipped_large.append({'eid': eid, 'stage': key, 'name': name, 'size': sz})
                    continue
                else:
                    take = sz
    ...
    shader_files = {}
    for stage, key in [(rd.ShaderStage.Vertex, 'VS229351'), (rd.ShaderStage.Pixel, 'PS229352')]:
        refl = s.GetShaderReflection(stage)
        shader_files[key] = {
    ...
    existing = {}
    search_roots = [
        os.path.join(ASSETS, 'ColourPass6_VS229351_PS229352_Batch', 'TextureDatabase'),
    ...
    for r, v in sorted(texture_refs.items()):
        is_unique = bool(UNIQUE_MATERIAL.intersection(v.get('bindings', []))) or bool(VS_SHARED.intersection(v.get('bindings', [])))
        if not is_unique:
            continue
        if existing.get(r):
    ...
    out = {
        ...
        'notes': {
            ...
            'instanceBuffer': 'VS uniforms27 stride 256 (forced; dump size heuristic is 96)',
            'localMaterialCB': 'PS uniforms30 / VS uniforms33 352B',
            'skinning': 'skip; flags at +76 are not skin bits',
            'raster': raster,
            'liveUnityVP': True,
        ...
        'statistics': {
            ...
            'pendingTextureSaves': len(pending),
    ...
    for r, v in sorted(texture_refs.items()):
        ...
        if unique and os.path.exists(local):
            ...
            if v['exportedAsset'] is None:
                hit = next((x['rel'] for x in pending if x['rid'] == r), None)
                if hit:
                    v['exportedAsset'] = hit
        out['textureDatabase'].append(v)

    open(os.path.join(ROOT, 'VS229351_PS229352_BatchManifest.json'), 'w', encoding='utf8').write(json.dumps(out, indent=2, default=str))
    result_path = r'D:/endcopy/EID3336_URP_GBuffer_Workspace/.rdctools/export_229351_229352_batch_result.json'
    summary = {
        'statistics': out['statistics'],
        'elapsedSeconds': out['elapsedSeconds'],
        'missingEIDs': missing,
        'exportedTextures': exported,
        'pendingTextureSaves': pending,
        ...
        'skippedLargeCBs': skipped_large,
        'instanceTranslation': instance_mats[0][0][3:] if instance_mats else None,
    }
