...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    internal class DeferredLights
    {
        ...
        internal int GBufferLightingIndex { get { return UseEID3336FiveMRT ? 5 : 3; } }
        ...
        internal int GBufferSliceCount { get { return (UseEID3336FiveMRT ? 6 : 4) + (UseRenderPass ? 1 : 0) + (UseShadowMask ? 1 : 0) + (UseRenderingLayers ? 1 : 0); } }

        internal GraphicsFormat GetGBufferFormat(int index)
        {
            if (UseEID3336FiveMRT && index < 5)
            {
                // EID3336 PS209987 emits RT0 B10G11R11, RT1-RT3 A2B10G10R10, RT4 RGBA8 sRGB.
                if (index == 0) return GraphicsFormat.B10G11R11_UFloatPack32;
                if (index == 1 || index == 2 || index == 3) return GraphicsFormat.A2B10G10R10_UNormPack32;
        ...
        // Source-level EID3336 path: five captured MRT color attachments plus a separate camera color target.
        internal bool UseEID3336FiveMRT { get; set; }
        //
        ...
        internal void CreateGbufferResources()
        {
            ...
            bool recreate = this.GbufferRTHandles == null || this.GbufferRTHandles.Length != gbufferSliceCount;
            if (!recreate && this.GbufferFormats != null && this.GbufferFormats.Length == gbufferSliceCount)
            {
                for (int i = 0; i < gbufferSliceCount; ++i)
                {
                    if (this.GbufferFormats[i] != this.GetGBufferFormat(i))
                    {
                        recreate = true;
                        break;
                    }
        ...
        {
            // Native render-pass input attachments exclude the lighting target.
            // The source five-MRT path is not supported with framebuffer fetch;
            // keep this mapping strictly on the stock four-attachment contract.
            ...
            RTHandle colorAttachment)
        {
            ...
            var inputCount = (UseEID3336FiveMRT ? 5 : 4) + (UseShadowMask ?  1 : 0);
