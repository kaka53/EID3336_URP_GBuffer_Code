#ifndef EID215541_215542_GBUFFER_INCLUDED
#define EID215541_215542_GBUFFER_INCLUDED
...
float4 _WindB;
float _WindGate;
float _EID215542MipBias;
CBUFFER_END

struct Attributes215541
{
...
float3 DecodeOctNormal215541(uint packed)
{
...
float4 DecodePackedTangent215541(uint packed, float3 n)
{
...
float3 RotateAroundAxis215541(float3 v, float3 axis, float radians)
{
    axis = normalize(axis);
    float s = sin(radians);
    float c = cos(radians);
...
float3 EvaluateWind215541(float3 posOS, float3 posWS, float3 instanceT, float instanceScale, float4 loc3, float3 windVecOS, float4 pack)
{
    ...
    float3 rotOff = RotateAroundAxis215541(rel, axis, radians(angleDeg)) - rel;
...
Varyings215541 EID215541Vertex(Attributes215541 input)
{
    Varyings215541 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 normalOS = packedBasis ? DecodeOctNormal215541(packed) : input.packedNormal.xyz;
    float4 tangentOS = packedBasis ? DecodePackedTangent215541(packed, normalOS) : input.input2;
    ...
    positionWS += EvaluateWind215541(input.position, positionWS, instanceT, instanceScale, input.input3, windVecOS, _WindA);
...
GBufferOutput215542 EID215542Fragment(Varyings215541 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215542 o;
