...
[InitializeOnLoad]
public static class ColourPass8PerEIDCapture
{
    ...
    const string TriggerPath = "Validation/ColourPass8Batch/CapturePerEID.request";
    const string OutputPath = "Validation/ColourPass8Batch/Captures/PerEID";
    static bool running;
    static ColourPass8PerEIDCapture() { EditorApplication.update += Poll; }
    static void Poll()
    {
    ...
    [MenuItem("Tools/Colour Pass 8/Capture Each EID RT4")]
    public static void CaptureAll()
    {
        ...
        try
        {
            ...
            if (groups.Length != 18) throw new InvalidOperationException("Expected 18 EID groups, got " + groups.Length);
            const int size = 384;
            mrt = new RenderTexture[5];
            for (int i = 0; i < 5; ++i) { mrt[i] = new RenderTexture(size, size, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear) { name = "ColourPass8_PerEID_RT" + i, hideFlags = HideFlags.HideAndDontSave }; mrt[i].Create(); }
            depth = new RenderTexture(size, size, 24, RenderTextureFormat.Depth, RenderTextureReadWrite.Linear) { name = "ColourPass8_PerEID_Depth", hideFlags = HideFlags.HideAndDontSave }; depth.Create();
            ...
            var report = new StringBuilder(); report.AppendLine("date=2026-09-11"); report.AppendLine("shader=VS209982_PS209983"); report.AppendLine("resolution=" + size + "x" + size);
            foreach (Transform group in groups)
            {
                ...
                CommandBuffer cmd = new CommandBuffer { name = "ColourPass8 " + group.name + " RT4 validation" };
            ...
            Debug.Log("[ColourPass8 PerEID] PASS: 18 EIDs -> " + output);
        }
