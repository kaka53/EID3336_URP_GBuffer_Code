...
Texture2D<float4> _EID215839VSRes29 : register(t2);
...
StructuredBuffer<EID215839Instance> _EID215839Instances;

float4x4 EID3863ObjectToWorldMatrix()
{
    EID215839Instance rec = _EID215839Instances[(uint)max(VSglXInstanceIndex, 0)];
    return float4x4(
...
void vert_main()
{
    float4x4 objectToWorld = EID3863ObjectToWorldMatrix();
    ...
    float4 objectPos = mul(objectToWorld, float4(VSX3, 1.0f));

    float3 localPos = VSX3;
    float2 absDelta = abs(objectPos.xz - _25_m132.zw);
    if (!any(absDelta > _25_m131.x))
    {
        float span = _25_m131.x * 2.0f;
        float2 uv = float2(((objectPos.x - _25_m132.z) / span) + 0.5f, ((objectPos.z - _25_m132.w) / span) + 0.5f);
        float4 mask = _EID215839VSRes29.SampleLevel(sampler_point_clamp, uv, 0.0f);
        if (mask.x > 0.0f && mask.y > 0.0f)
            localPos = asfloat(0x7fc00000u).xxx;
    }
    ...
    float3 billboard = float3(unity_MatrixInvV[0].y, 0.0f, unity_MatrixInvV[2].y) * (VSX3.y * _EID215839Billboard);
    float3 worldPos = mul(basis, localPos) + translation + billboard;
...
EID3863VertexVaryings EID3863VertexMain(EID3863VertexInput i)
{
    EID3863VSInputInternal x;
    x.VSX3 = i.position;
    x.VSX4 = i.normal;
    x.VSX5 = i.tangent;
    x.VSX7 = i.uv0;
    x.VSglXInstanceIndex = (uint)i.instanceIndex;
    return EID3863VSGeneratedMain(x);
}
