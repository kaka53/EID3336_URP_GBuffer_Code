#ifndef EID230646_230647_GBUFFER_INCLUDED
#define EID230646_230647_GBUFFER_INCLUDED
...
float _EID230647MipBias;
float _EID230647GlobalY;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes230646
{
...
float3 DecodeOctNormal230646(uint packed)
{
...
float4 DecodePackedTangent230646(uint packed, float3 n)
{
...
Varyings230646 EID230646Vertex(Attributes230646 input)
{
    Varyings230646 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal230646(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent230646(packed, decodedNormalOS) : input.input2;
...
GBufferOutput230647 EID230647Fragment(Varyings230646 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput230647 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float mip = _EID230647MipBias;
    ...
    float4 layerSample = SAMPLE_TEXTURE2D_BIAS(_Res31, sampler_Res31, input.uv1, mip);
    ...
    color *= lerp(1.0, _EID230647GlobalY, _P01.w);

    float w0 = layerSample.x * _P17.w;
    float w1 = layerSample.y * _P18.w * _P16.x;
    float w2 = layerSample.z * _P19.w * _P16.x;
    float rem = 1.0 - w0 - w1 - w2;
    float3 layerRgb = rem * baseAlbedo + w0 * _P17.rgb + w1 * _P18.rgb + w2 * _P19.rgb;
    float roughness = lerp(roughnessLo, lerp(0.0, 1.0, _P16.y), layerSample.w * _P16.z);
