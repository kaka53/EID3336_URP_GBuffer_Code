#ifndef EID215525_215526_GBUFFER_INCLUDED
#define EID215525_215526_GBUFFER_INCLUDED
...
float _EID215526MipBias;
CBUFFER_END

struct Attributes215525
{
...
float3 DecodeOctNormal215525(uint packed)
{
...
float4 DecodePackedTangent215525(uint packed, float3 n)
{
...
float2 FilterNormalXY215525(float2 raw)
{
    float2 nxy = raw * 2.0 - 1.0;
    return float2(abs(nxy.x) < 0.012 ? 0.0 : nxy.x, abs(nxy.y) < 0.012 ? 0.0 : nxy.y);
}

Varyings215525 EID215525Vertex(Attributes215525 input)
{
    Varyings215525 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 normalOS = packedBasis ? DecodeOctNormal215525(packed) : input.packedNormal.xyz;
    float4 tangentOS = packedBasis ? DecodePackedTangent215525(packed, normalOS) : input.input2;
...
GBufferOutput215526 EID215526Fragment(Varyings215525 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215526 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res28, sampler_Res28, uvBase, _EID215526MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res30, sampler_Res30, uvNormal, _P03.y + _EID215526MipBias);

    float2 nxy = FilterNormalXY215525(normalSample.xy);
    ...
    float3 detail = SAMPLE_TEXTURE2D_BIAS(_Res32, sampler_Res32, uvOverlay, _EID215526MipBias).xyz;
    ...
    float4 extraColorSample = SAMPLE_TEXTURE2D_BIAS(_Res35, sampler_Res35, extraUV, _EID215526MipBias);
    ...
    float4 extraNSample = SAMPLE_TEXTURE2D_BIAS(_Res36, sampler_Res36, extraUV, _EID215526MipBias);
    ...
    float mask;
    if (_P28.y < 0.5)
        mask = SAMPLE_TEXTURE2D_BIAS(_Res33, sampler_Res33, maskUV, _EID215526MipBias).x;
    else
    {
    ...
    float extra34 = SAMPLE_TEXTURE2D_BIAS(_Res34, sampler_Res34, extraUV, _EID215526MipBias).x;
    float2 pair = float2(1.0 - mask, mask) * float2(extra34, extraColorSample.a);
