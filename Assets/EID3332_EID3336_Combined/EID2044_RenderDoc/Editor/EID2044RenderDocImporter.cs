...
[InitializeOnLoad]
public static class EID2044RenderDocImporter
{
    const string Root = "Assets/EID3332_EID3336_Combined/EID2044_RenderDoc";
    const string Captured = Root + "/Captured/";
    const string MeshPath = Root + "/Geometry/EID2044_VSInput_Exact.asset";
    const string MaterialPath = Root + "/Materials/EID2044_PS209977.mat";
    const string TargetScene = "Assets/EID3332_EID3336_Combined/Scenes/EID3336_RenderDocCamera.unity";
    const string RootObjectName = "EID2044_RenderDoc";
    const string ObjectName = "eid2044_instance_000";
    const int VertexCount = 6769;
    const int IndexCount = 12279;
    static readonly string Project = Directory.GetParent(Application.dataPath).FullName;
    static readonly string RequestPath = Path.Combine(Project, "Validation/ImportEID2044.request");
    static readonly string ReportPath = Path.Combine(Project, "Validation/EID2044/import_report.txt");
    static bool busy;

    static EID2044RenderDocImporter() { EditorApplication.update += Poll; }
    ...
    [MenuItem("Tools/EID3332+3336/Import EID2044 Into RenderDoc Camera Scene")]
    public static void ImportIntoCurrentScene()
    {
        ...
        try
        {
            ...
            if (stream0.Length != VertexCount * 16 || stream1.Length != VertexCount * 8 || constant.Length != 20 || indexBytes.Length != IndexCount * 2)
                throw new InvalidDataException("EID2044 captured stream sizes do not match the RenderDoc draw.");
            ...
            AssetDatabase.ImportAsset("Assets/EID3332_EID3336_Combined/EID2012_RenderDoc/Shaders/EID2012RenderDocGBuffer.shader", ImportAssetOptions.ForceUpdate | ImportAssetOptions.ForceSynchronousImport);
            Shader shader = AssetDatabase.LoadAssetAtPath<Shader>("Assets/EID3332_EID3336_Combined/EID2012_RenderDoc/Shaders/EID2012RenderDocGBuffer.shader");
            if (!shader || !shader.isSupported || ShaderUtil.ShaderHasError(shader))
                throw new InvalidOperationException("Shared VS209975/PS209977 shader is missing, unsupported, or has compile errors.");

            Texture2D baseColor = ImportBC7("PS_res17_rid270221", false);
            Texture2D normalMaterial = ImportBC7("PS_res19_rid270228", false);
            var generatedMaterial = new Material(shader) { name = "EID2044_PS209977" };
            ...
            go.transform.localPosition = new Vector3(-353.27154541015625f, 107.83035278320312f, -481.44366455078125f);
            ...
            report.AppendLine("event=2044");
            report.AppendLine("draw=12279 indices, 1 instance, 6769 vertices, 4093 triangles");
            report.AppendLine("VS=209975; PS=209977");
            report.AppendLine("position=(-353.27154541015625, 107.83035278320312, -481.44366455078125)");
            report.AppendLine("rotation=(0,0,0,1); scale=(1,1,1)");
            report.AppendLine("baseColor=res17/RID270221/BC7_SRGB; normalMaterial=res19/RID270228/BC7_SRGB");
            ...
            Debug.Log("[EID2044] Import complete: " + report.ToString());
        }
        finally { busy = false; }
    }
    ...
    static Mesh BuildExactMesh(byte[] s0, byte[] s1, byte[] constant, byte[] ib)
    {
        ...
        for (int i = 0; i < indices.Length; i++) if (indices[i] >= VertexCount) throw new InvalidDataException("EID2044 index out of range at " + i);

        var mesh = new Mesh { name = "EID2044 VSInput Exact (6769 vertices, 12279 indices)", indexFormat = IndexFormat.UInt16 };
