#ifndef EID229074_229075_GBUFFER_INCLUDED
#define EID229074_229075_GBUFFER_INCLUDED
...
float _EID229075MipBias;
float _Global75W;
float _LightMixY;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes229074
{
...
float3 DecodeOctNormal229074(uint packed)
{
...
float4 DecodePackedTangent229074(uint packed, float3 n)
{
...
Varyings229074 EID229074Vertex(Attributes229074 input)
{
    Varyings229074 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal229074(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent229074(packed, decodedNormalOS) : input.input2;
...
GBufferOutput229075 EID229075Fragment(Varyings229074 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput229075 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvBase, _EID229075MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res25, sampler_Res25, uvNormal, _P03.y + _EID229075MipBias);
    float4 materialSample = SAMPLE_TEXTURE2D_BIAS(_Res27, sampler_Res27, uvNormal, _EID229075MipBias);
    ...
    float motionWeight = lerp(_P07.z, 0.7, active);
    ...
    o.rt2 = float4(materialY, ao, 0.0, float(materialId / 4u) * 0.3333333433);
