#ifndef EID215547_215548_GBUFFER_INCLUDED
#define EID215547_215548_GBUFFER_INCLUDED
...
float _EID215548MipBias;
CBUFFER_END

struct Attributes215547
{
...
float3 DecodeOctNormal215547(uint packed)
{
...
float4 DecodePackedTangent215547(uint packed, float3 n)
{
...
Varyings215547 EID215547Vertex(Attributes215547 input)
{
    Varyings215547 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 normalOS = packedBasis ? DecodeOctNormal215547(packed) : input.packedNormal.xyz;
    float4 tangentOS = packedBasis ? DecodePackedTangent215547(packed, normalOS) : input.input2;
...
GBufferOutput215548 EID215548Fragment(Varyings215547 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215548 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = _Res34.SampleBias(sampler_Res34, uvBase, _EID215548MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 packedN = _Res36.SampleBias(sampler_Res36, uvNormal, _P03.y + _EID215548MipBias);
    ...
    float4 packedM = _Res38.SampleBias(sampler_Res38, uvNormal, _EID215548MipBias);
    ...
    if (axis == 0u)
        ...
        extraUV = input.uv2;
    extraUV = extraUV * _P23.z + _P28.xy;

    float4 extraColorSample = _Res41.SampleBias(sampler_Res41, extraUV, _EID215548MipBias);
    float luma = dot(extraColorSample.rgb, float3(0.2127, 0.7152, 0.0722));
    float3 extraColor = lerp(luma.xxx, extraColorSample.rgb, saturate(_P25.w + 1.0)) * _P27.rgb * _P26.x;
    float4 extraNSample = _Res42.SampleBias(sampler_Res42, extraUV, _EID215548MipBias);
    float4 extraNPacked = float4(extraNSample.x, extraNSample.y, 0.0, extraNSample.x);
    float2 en = extraNPacked.wy * 2.0 - 1.0;
    float extraNz = max(0.0, sqrt(saturate(1.0 - saturate(dot(en, en)))));
    float3 extraTS = float3(en * _P23.w, extraNz);

    float mask = saturate(1.0 - input.extra.x);
    float extraA = _P24.y != 0.0 ? extraColorSample.a : _P24.x;
    float extraAO = _P25.z != 0.0 ? 1.0 : lerp(1.0, extraNSample.w, _P24.z);
    float extraBlendSample = _Res40.SampleBias(sampler_Res40, extraUV, _EID215548MipBias).x;
    float2 pair = float2(1.0 - mask, mask) * float2(extraBlendSample, extraColorSample.a);
    float pairMax = max(pair.x, pair.y);
    float2 t = max(0.0.xx, pair + _P23.y.xx - pairMax.xx);
    t *= float2(1.0 - mask, mask);
    float extraBlend = max(0.0, t.x + t.y);
    float2 nrm = extraBlend > 0.0 ? t / extraBlend : 0.0.xx;
    float extraMask = _P25.y != 0.0 ? nrm.y : mask;
    ...
    float3 extraBlended = lerp(extraTS, reoriented, _P24.w.xxx);
    ...
    materialZ *= step(extraMask, 1.0 - _P25.x);
