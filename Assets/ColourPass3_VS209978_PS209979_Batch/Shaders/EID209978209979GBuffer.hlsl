#ifndef EID209978_209979_GBUFFER_INCLUDED
#define EID209978_209979_GBUFFER_INCLUDED
...
float _EID209979MipBias;
CBUFFER_END

struct Attributes209978
{
...
float3 DecodeOctNormal209978(uint packed)
{
...
float4 DecodePackedTangent209978(uint packed,float3 n)
{
...
Varyings209978 EID209978Vertex(Attributes209978 input)
{
    Varyings209978 o;
    uint packed=asuint(input.positionPacked.w);
    bool packedBasis=(packed&0x40000000u)!=0u;
    float3 normalOS=packedBasis?DecodeOctNormal209978(packed):float3(input.positionPacked.w,0.0,1.0);
    float4 tangentOS=packedBasis?DecodePackedTangent209978(packed,normalOS):input.input2;
...
struct GBufferOutput209979 { float4 rt0:SV_Target0; float4 rt1:SV_Target1; float4 rt2:SV_Target2; float4 rt3:SV_Target3; float4 rt4:SV_Target4; };
GBufferOutput209979 EID209979Fragment(Varyings209978 input,bool isFrontFace:SV_IsFrontFace)
{
    GBufferOutput209979 o;
    float tangentSign=input.tangentWS.w>0.0?1.0:-1.0;
    float2 uvBase=lerp(input.uv0,input.uv1,_P02.w)*_P11.xy+_P11.zw;
    float2 uvMaterial=lerp(input.uv0,input.uv1,_P03.x)*_P12.xy+_P12.zw;
    float4 baseSample=SAMPLE_TEXTURE2D_BIAS(_Res23,sampler_Res23,uvBase,_EID209979MipBias);
    float4 normalSample=SAMPLE_TEXTURE2D_BIAS(_Res25,sampler_Res25,uvMaterial,_P03.y+_EID209979MipBias);
    normalSample.w*=normalSample.x;
    float4 materialSample=SAMPLE_TEXTURE2D_BIAS(_Res27,sampler_Res27,uvMaterial,_EID209979MipBias);
    float2 rawNormalXY=normalSample.wy*2.0-1.0;
    float2 normalXY=rawNormalXY*_P00.x;
    ...
    float tangentZ=max(1.0000000168623835e-16,sqrt(1.0-saturate(dot(rawNormalXY,rawNormalXY))))*faceSign;
    ...
    o.rt2=float4(materialY,lerp(1.0,materialSample.z,_P01.x),materialZ,float(materialId/4u)*0.3333333433);
