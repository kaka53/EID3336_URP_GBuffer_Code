...
[InitializeOnLoad]
public static class ColourPass3VS209978PS209979Capture
{
    ...
    const string TriggerPath = "Validation/ColourPass3Batch/Capture.request";
    const string OutputPath = "Validation/ColourPass3Batch/Captures";
    static bool running;

    static ColourPass3VS209978PS209979Capture() { EditorApplication.update += Poll; }
    static void Poll()
    {
    ...
    [MenuItem("Tools/Colour Pass 3/Capture VS209978 PS209979 Batch")]
    public static void Capture()
    {
        ...
        try
        {
            ...
            if (renderers.Length != 81) throw new InvalidOperationException("Expected 81 batch renderers, got " + renderers.Length);
            ...
            {
                name = "ColourPass3_Depth",
                hideFlags = HideFlags.HideAndDontSave
            };
            depth.Create();

            var colors = mrt.Select(x => new RenderTargetIdentifier(x)).ToArray();
            CommandBuffer cmd = new CommandBuffer { name = "ColourPass3 VS209978 PS209979 Direct Five MRT Validation" };
            ...
            foreach (Renderer r in renderers)
            {
                ...
                if (material.shader == null || material.shader.name != "EID/URP/VS209978_PS209979_GBuffer")
            ...
            Debug.Log("[ColourPass3 Capture] PASS: " + output + " nonBlack=" + string.Join(",", nonBlack));
        }
        catch (Exception e)
        {
