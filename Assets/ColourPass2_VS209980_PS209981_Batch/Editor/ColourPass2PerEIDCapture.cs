...
[InitializeOnLoad]
public static class ColourPass2PerEIDCapture
{
    const string ScenePath = "Assets/EID3332_EID3336_Combined/Scenes/EID3336_RenderDocCamera.unity";
    const string RootName = "ColourPass2_VS209980_PS209981";
    const string TriggerPath = "Validation/ColourPass2Batch/CapturePerEID.request";
    const string OutputPath = "Validation/ColourPass2Batch/Captures/PerEID";
    static bool running;
    static ColourPass2PerEIDCapture() { EditorApplication.update += Poll; }
    static void Poll()
    {
    ...
    [MenuItem("Tools/Colour Pass 2/Capture Each EID RT4")]
    public static void CaptureAll()
    {
        ...
        try
        {
            ...
            if (groups.Length != 35) throw new InvalidOperationException("Expected 35 EID groups, got " + groups.Length);
            const int size = 384;
            mrt = new RenderTexture[5];
            for (int i = 0; i < 5; ++i) { mrt[i] = new RenderTexture(size, size, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear) { name = "ColourPass2_PerEID_RT" + i, hideFlags = HideFlags.HideAndDontSave }; mrt[i].Create(); }
            depth = new RenderTexture(size, size, 24, RenderTextureFormat.Depth, RenderTextureReadWrite.Linear) { name = "ColourPass2_PerEID_Depth", hideFlags = HideFlags.HideAndDontSave }; depth.Create();
            ...
            var report = new StringBuilder(); report.AppendLine("date=2026-09-10"); report.AppendLine("shader=VS209980_PS209981"); report.AppendLine("resolution=" + size + "x" + size);
            foreach (Transform group in groups)
            {
                ...
                CommandBuffer cmd = new CommandBuffer { name = "ColourPass2 " + group.name + " RT4 validation" };
            ...
            Debug.Log("[ColourPass2 PerEID] PASS: 35 EIDs -> " + output);
        }
