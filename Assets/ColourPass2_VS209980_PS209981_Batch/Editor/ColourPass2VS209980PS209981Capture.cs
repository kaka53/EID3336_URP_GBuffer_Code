...
[InitializeOnLoad]
public static class ColourPass2VS209980PS209981Capture
{
    ...
    const string TriggerPath = "Validation/ColourPass2Batch/Capture.request";
    const string OutputPath = "Validation/ColourPass2Batch/Captures";
    static bool running;

    static ColourPass2VS209980PS209981Capture() { EditorApplication.update += Poll; }
    static void Poll()
    {
    ...
    [MenuItem("Tools/Colour Pass 2/Capture VS209980 PS209981 Batch")]
    public static void Capture()
    {
        ...
        try
        {
            ...
            if (renderers.Length != 91) throw new InvalidOperationException("Expected 91 batch renderers, got " + renderers.Length);
            ...
            {
                name = "ColourPass2_Depth",
                hideFlags = HideFlags.HideAndDontSave
            };
            depth.Create();

            var colors = mrt.Select(x => new RenderTargetIdentifier(x)).ToArray();
            CommandBuffer cmd = new CommandBuffer { name = "ColourPass2 VS209980 PS209981 Direct Five MRT Validation" };
            ...
            foreach (Renderer r in renderers)
            {
                ...
                if (material.shader == null || material.shader.name != "EID/URP/VS209980_PS209981_GBuffer")
            ...
            Debug.Log("[ColourPass2 Capture] PASS: " + output + " nonBlack=" + string.Join(",", nonBlack));
        }
        catch (Exception e)
        {
