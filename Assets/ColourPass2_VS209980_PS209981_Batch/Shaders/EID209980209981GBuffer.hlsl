#ifndef EID209980_209981_GBUFFER_INCLUDED
#define EID209980_209981_GBUFFER_INCLUDED

#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"

TEXTURE2D(_Res23); SAMPLER(sampler_Res23); // RenderDoc res23 / binding 3: base colour
TEXTURE2D(_Res25); SAMPLER(sampler_Res25); // RenderDoc res25 / binding 2: tangent-space normal/material
...
float _EID209981MipBias;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes209980
{
    float4 positionPacked : POSITION;    // _input0.xyz + exact _input1.x bit pattern
    float4 input2 : COLOR0;              // _input2 R8G8B8A8_UNORM
    float4 input3 : TANGENT;             // _input3 R8G8B8A8_UNORM
    float2 input4 : TEXCOORD0;
    float2 input5 : TEXCOORD1;
    float2 input6 : TEXCOORD2;
    float4 input7 : TEXCOORD3;           // _input7 R8G8B8A8_UNORM
    uint4 input8 : BLENDINDICES0;         // _input8 R8G8B8A8_UINT
    float3 bakedNormalOS : TEXCOORD4;
    float4 bakedTangentOS : TEXCOORD5;
};

struct Varyings209980
{
...
float3 DecodeOctNormal209980(uint packed)
{
...
float4 DecodePackedTangent209980(uint packed, float3 n)
{
...
Varyings209980 EID209980Vertex(Attributes209980 input)
{
    Varyings209980 o;
    uint packed = asuint(input.positionPacked.w);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal209980(packed) : float3(input.positionPacked.w, 0.0, 1.0);
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent209980(packed, decodedNormalOS) : input.input2;
...
GBufferOutput209981 EID209981Fragment(Varyings209980 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput209981 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvBase, _EID209981MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res25, sampler_Res25, uvNormal, _P03.y + _EID209981MipBias);

    float2 normalXY = normalSample.xy * 2.0 - 1.0;
    normalXY = float2(abs(normalXY.x) < 0.012 ? 0.0 : normalXY.x,
                      abs(normalXY.y) < 0.012 ? 0.0 : normalXY.y) * _P00.x;
    ...
    float tangentZ = sqrt(saturate(1.0 - dot(normalXY, normalXY))) * faceSign;
