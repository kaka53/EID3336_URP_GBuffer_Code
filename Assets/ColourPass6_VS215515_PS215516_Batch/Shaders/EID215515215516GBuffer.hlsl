#ifndef EID215515_215516_GBUFFER_INCLUDED
#define EID215515_215516_GBUFFER_INCLUDED
...
float4 _P36; float4 _P37;
float4 _InstanceMeta;
float4 _InstanceStateYZ;
float _EID215516MipBias;
CBUFFER_END

struct Attributes215515
{
...
float3 DecodeOctNormal215515(uint packed)
{
...
float4 DecodePackedTangent215515(uint packed, float3 n)
{
...
float2 FilterNormalXY215515(float2 raw)
{
    float2 nxy = raw * 2.0 - 1.0;
    return float2(abs(nxy.x) < 0.012 ? 0.0 : nxy.x, abs(nxy.y) < 0.012 ? 0.0 : nxy.y);
}

Varyings215515 EID215515Vertex(Attributes215515 input)
{
    Varyings215515 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 normalOS = packedBasis ? DecodeOctNormal215515(packed) : input.packedNormal.xyz;
    float4 tangentOS = packedBasis ? DecodePackedTangent215515(packed, normalOS) : input.input2;
...
GBufferOutput215516 EID215516Fragment(Varyings215515 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215516 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res32, sampler_Res32, uvBase, _EID215516MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res34, sampler_Res34, uvNormal, _P03.y + _EID215516MipBias);

    float2 nxy = FilterNormalXY215515(normalSample.xy);
    ...
    float4 detailN = SAMPLE_TEXTURE2D_BIAS(_Res37, sampler_Res37, uvDetailN, _EID215516MipBias);
    ...
    float2 dxy = FilterNormalXY215515(detailN.xy);
    ...
    float3 detail = SAMPLE_TEXTURE2D_BIAS(_Res36, sampler_Res36, uvOverlay, _EID215516MipBias).xyz;
    float3 lo = float3(_P29.x, _P29.z, _P30.x);
    float3 hi = float3(_P29.y, _P29.w, _P30.y);
    float3 weights = saturate(lo * 0.5 + lerp(-lo, 1.0, detail + hi)) * float3(_P31.w, _P32.w, _P33.w);
    float4 layered = float4(baseColor, roughness);
    layered = lerp(layered, float4(_P33.xyz, _P27.w), weights.z);
    layered = lerp(layered, float4(_P32.xyz, _P27.x), weights.y);
    layered = lerp(layered, float4(_P31.xyz, _P26.y), weights.x);
    baseColor = layered.xyz;
    roughness = layered.w;
    materialY = lerp(materialY, _P28.z, weights.z);
    materialY = lerp(materialY, _P28.y, weights.y);
    materialY = lerp(materialY, _P28.x, weights.x);
