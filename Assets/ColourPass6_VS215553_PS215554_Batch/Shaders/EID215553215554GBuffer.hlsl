#ifndef EID215553_215554_GBUFFER_INCLUDED
#define EID215553_215554_GBUFFER_INCLUDED
...
float _EID215554MipBias;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215553
{
...
float3 DecodeOctNormal215553(uint packed)
{
...
float4 DecodePackedTangent215553(uint packed, float3 n)
{
...
Varyings215553 EID215553Vertex(Attributes215553 input)
{
    Varyings215553 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215553(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215553(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215554 EID215554Fragment(Varyings215553 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215554 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float mip = _EID215554MipBias;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvBase, mip);
    ...
    float alpha = lerp(baseSample.w, normalSample.w, _P01.z) * _P08.w;
    clip(alpha - _P22.x);
