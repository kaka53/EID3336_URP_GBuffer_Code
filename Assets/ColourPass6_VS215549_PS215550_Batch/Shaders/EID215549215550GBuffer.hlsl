#ifndef EID215549_215550_GBUFFER_INCLUDED
#define EID215549_215550_GBUFFER_INCLUDED
...
float _EID215550MipBias;
float _LightMixY;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215549
{
...
float3 DecodeOctNormal215549(uint packed)
{
...
float4 DecodePackedTangent215549(uint packed, float3 n)
{
...
Varyings215549 EID215549Vertex(Attributes215549 input)
{
    Varyings215549 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215549(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215549(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215550 EID215550Fragment(Varyings215549 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215550 o;
    ...
    float4 albedoSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvAlbedo, _EID215550MipBias);
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res25, sampler_Res25, uvNormal, _P03.y + _EID215550MipBias);
    float4 extraSample = SAMPLE_TEXTURE2D_BIAS(_Res27, sampler_Res27, uvNormal, _EID215550MipBias);
    ...
    float overlayMode = _P26.y;
    float overlaySelect = saturate(overlayMode);
    float overlayFromW = saturate(overlayMode - 2.0);
    float overlayChannel = lerp(albedoSample.w, extraSample.w, overlayFromW);
    float overlayMask = saturate(lerp(1.0, overlayChannel, overlaySelect) * 1.1111 - 0.0556);
    float3 overlayTint = lerp(_P25.rgb, _OverlayChild7.rgb, _OverlayChild7.aaa);
    float3 overlayColor = lerp(0.0.xxx, overlayTint * overlayMask * _P26.x, overlaySelect.xxx);
