#ifndef EID215539_215540_GBUFFER_INCLUDED
#define EID215539_215540_GBUFFER_INCLUDED
...
float _EID215540MipBias;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215539
{
...
float3 DecodeOctNormal215539(uint packed)
{
...
float4 DecodePackedTangent215539(uint packed, float3 n)
{
...
float2 FilterNormalXY215539(float2 raw)
{
    float2 nxy = raw * 2.0 - 1.0;
    return abs(nxy) < 0.012.xx ? 0.0.xx : nxy;
}

Varyings215539 EID215539Vertex(Attributes215539 input)
{
    Varyings215539 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215539(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215539(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215540 EID215540Fragment(Varyings215539 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215540 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float mip = _EID215540MipBias;
    ...
    float2 nxy = FilterNormalXY215539(normalSample.xy);
    ...
    float2 uvExtra = lerp(input.uv0, input.uv1, _P24.z) * _P26.xy + _P26.zw;
    float4 extraSample = SAMPLE_TEXTURE2D_BIAS(_Res31, sampler_Res31, uvExtra, mip);
    float extraSrc = lerp(1.0, extraSample.w, saturate(_P23.y));
    float extraAlt = lerp(lerp(albedoSample.a, normalSample.z, saturate(_P23.y - 2.0)), normalSample.w, saturate(_P23.y - 3.0));
    float extraMask = lerp(extraSrc, extraAlt, saturate(_P23.y - 1.0));
    float viewAbsZ = abs(TransformWorldToView(input.positionWS).z);
    extraMask *= saturate((_P24.y - viewAbsZ) / max(_P24.y - _P24.x, 1e-8));

    float2 exy = FilterNormalXY215539(extraSample.xy);
    float3 extraTS = float3(exy * (extraMask * _P23.z), sqrt(saturate(1.0 - dot(exy, exy))));
    float3 basePlus = baseTS + float3(0.0, 0.0, 1.0);
    float3 extraFlip = extraTS * float3(-1.0, -1.0, 1.0);
    float3 tangentN = (basePlus * dot(basePlus, extraFlip)) / max(basePlus.z, 0.0) - extraFlip;

    float extraBlend = _P23.w * extraMask;
    roughness = lerp(roughness, lerp(extraSample.w, extraSample.z, _P23.x), extraBlend);
    ao *= lerp(1.0, lerp(1.0, extraSample.w, _P23.x), extraBlend);
    float extraColorAmt = extraMask * (1.0 - _P23.x) * (1.0 - extraSample.z);
    float3 extraCol = lerp(saturate(baseColor * _P25.rgb * _P24.w), _P25.rgb, _P25.w);
    baseColor = lerp(baseColor, extraCol, extraColorAmt);
