...
// Single reconstructed plant: instance matrix comes from the GameObject Transform.

cbuffer EID3863WindParameters : register(b4)
{
...
SamplerState sampler_linear_repeat : register(s8);
SamplerState sampler_point_clamp : register(s10);
SamplerState sampler_linear_clamp : register(s9);
Texture2D<float4> _EID3863VSRes34 : register(t2);
Texture2D<float4> _EID3863VSRes35 : register(t3);
Texture2D<float4> _EID3863VSRes36 : register(t1);
...
StructuredBuffer<EID215849Instance> _EID215849Instances;

float4x4 EID3863ObjectToWorldMatrix()
{
    EID215849Instance rec = _EID215849Instances[(uint)max(VSglXInstanceIndex, 0)];
    return float4x4(
...
void vert_main()
{
    float4 _188 = mul(EID3863ObjectToWorldMatrix(), float4(VSX3, 1.0f));
    float3 _189 = _188.xyz;
    float3 _193 = float3(EID3863ObjectToWorldMatrix()[0].w, EID3863ObjectToWorldMatrix()[1].w, EID3863ObjectToWorldMatrix()[2].w);
    float3 _201 = _189 - _193;
    _201.y = 0.0f;
    float3 _212 = ((_193 + (_201 * _39_m24)) * (1.0f - _39_m25)) + (mul(EID3863ObjectToWorldMatrix(), float4(VSX8, VSX9.x, 1.0f)).xyz * _39_m25);
    ...
    if (_349)
    {
        ...
        if (_25_m35[0].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _405 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[0].y);
                if (_25_m34[0].w != 0.0f)
                {
    ...
    if (_463)
    {
        ...
        if (_25_m35[1].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _522 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[1].y);
                if (_25_m34[1].w != 0.0f)
                {
    ...
    if (_583)
    {
        ...
        if (_25_m35[2].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _642 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[2].y);
                if (_25_m34[2].w != 0.0f)
                {
    ...
    if (_703)
    {
        ...
        if (_25_m35[3].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _762 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[3].y);
                if (_25_m34[3].w != 0.0f)
                {
    ...
    do
    {
        ...
        float4 _885 = _EID3863VSRes35.SampleLevel(sampler_linear_repeat, _876 + 0.5f.xx, 0.0f);
        float _886 = _885.x;
        ...
        float4 _921 = ((float4(_885.yy, _EID3863VSRes35.SampleLevel(sampler_linear_repeat, _876 + float2(0.50390625f, 0.5f), 0.0f).y, _EID3863VSRes35.SampleLevel(sampler_linear_repeat, _876 + float2(0.5f, 0.50390625f), 0.0f).y) - 0.5f.xxxx) * 10.0f.xxxx) + _25_m41.yyyy;
    ...
    if (_349)
    {
        ...
        if (_25_m35[0].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _1106 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[0].y);
                if (_25_m34[0].w != 0.0f)
                {
    ...
    if (_463)
    {
        ...
        if (_25_m35[1].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _1222 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[1].y);
                if (_25_m34[1].w != 0.0f)
                {
    ...
    if (_583)
    {
        ...
        if (_25_m35[2].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _1341 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[2].y);
                if (_25_m34[2].w != 0.0f)
                {
    ...
    if (_703)
    {
        ...
        if (_25_m35[3].x != 0.0f)
        {
            ...
            do
            {
                ...
                float _1460 = abs(EID3863ObjectToWorldMatrix()[1].w - _25_m32[3].y);
                if (_25_m34[3].w != 0.0f)
                {
    ...
    do
    {
        ...
        float4 _1573 = _EID3863VSRes36.SampleLevel(sampler_linear_repeat, _1564 + 0.5f.xx, 0.0f);
        float _1574 = _1573.x;
        ...
        float4 _1609 = ((float4(_1573.yy, _EID3863VSRes36.SampleLevel(sampler_linear_repeat, _1564 + float2(0.50390625f, 0.5f), 0.0f).y, _EID3863VSRes36.SampleLevel(sampler_linear_repeat, _1564 + float2(0.5f, 0.50390625f), 0.0f).y) - 0.5f.xxxx) * 10.0f.xxxx) + _25_m42.yyyy;
    ...
    float3 _1678 = float3(EID3863_RD_CAMERA_RIGHT_Y, 0.0f, EID3863_RD_CAMERA_FORWARD_Y) * (VSX3.y * _39_m6);
    ...
    else
    {
        ...
        float4 _1712 = _EID3863VSRes34.SampleLevel(sampler_point_clamp, float2(((_188.x - _25_m132.z) / _1699) + 0.5f, ((_188.z - _25_m132.w) / _1699) + 0.5f), 0.0f);
        float _1713 = _1712.x;
    ...
    float3x3 _1830 = float3x3(EID3863ObjectToWorldMatrix()[0].xyz, EID3863ObjectToWorldMatrix()[1].xyz, EID3863ObjectToWorldMatrix()[2].xyz);
    float3 _1838 = mul(_1830, _1821);
    float3 _1844 = mul(_1830, _1823.xyz);
    // RenderDoc computes the deformed local vertex first.  Unity supplies the live object and camera matrices.
    float3 EID3863WorldPosition = (mul(_1830, _1736) + _193) + (((((_212 + ((_853 * rsqrt(max(1.1754943508222875079687365372222e-38f, dot(_853, _853)))) * _859)) - _189) + _980) + _1678) * _1740);
    float4 _1860 = mul(UNITY_MATRIX_VP, float4(EID3863WorldPosition, 1.0f));
    VSglXPosition = _1860;
    VSX12 = VSX7;
    ...
    VSX16 = _1860.xyw;
    // Same current-frame clip coordinate prevents previous-frame motion residue.
    VSX17 = _1860.xyw;
    VSX18 = _1736;
    VSX19 = uint(VSglXInstanceIndex);
}

EID3863VertexVaryings EID3863VSGeneratedMain(EID3863VSInputInternal stage_input)
{
    ...
    vert_main();
    EID3863VertexVaryings stage_output;
...
struct EID3863VertexInput { float3 position : POSITION; float3 normal : NORMAL; float4 tangent : TANGENT; float4 color : COLOR; float2 uv0 : TEXCOORD0; float2 uv1 : TEXCOORD1; float2 uv2 : TEXCOORD2; float instanceIndex : TEXCOORD3; };
EID3863VertexVaryings EID3863VertexMain(EID3863VertexInput i) { EID3863VSInputInternal x; x.VSX3=i.position; x.VSX4=i.normal; x.VSX5=i.tangent; x.VSX6=i.color; x.VSX7=i.uv0; x.VSX8=i.uv1; x.VSX9=i.uv2; x.VSglXInstanceIndex=(uint)i.instanceIndex; return EID3863VSGeneratedMain(x); }
