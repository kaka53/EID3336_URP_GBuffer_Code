#ifndef EID215493_215494_GBUFFER_INCLUDED
#define EID215493_215494_GBUFFER_INCLUDED
...
float4 _OverlayChild5;
float4 _OverlayChild7;
float4 _OverlayChild9;
float _EID215494MipBias;
float _LightMixY;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215493
{
...
float3 DecodeOctNormal215493(uint packed)
{
...
float4 DecodePackedTangent215493(uint packed, float3 n)
{
...
Varyings215493 EID215493Vertex(Attributes215493 input)
{
    Varyings215493 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215493(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215493(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215494 EID215494Fragment(Varyings215493 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215494 o;
    ...
    float4 albedoSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvAlbedo, _EID215494MipBias);
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res25, sampler_Res25, uvNormal, _P03.y + _EID215494MipBias);
    ...
    float overlayMode = _P25.y;
    float overlaySelect = saturate(overlayMode);
    float overlayFromZ = saturate(overlayMode - 1.0);
    float overlayFromW = saturate(overlayMode - 2.0);
    float overlayChannel = lerp(albedoSample.w, lerp(normalSample.z, normalSample.w, overlayFromW), overlayFromZ);
    float overlayMask = saturate(lerp(1.0, overlayChannel, overlaySelect) * 1.1111 - 0.0556);
    float3 overlayTint = lerp(_P24.rgb, _OverlayChild7.rgb, _OverlayChild7.aaa);
    float3 overlayColor = lerp(0.0.xxx, overlayTint * overlayMask * _P25.x, overlaySelect.xxx);
    float overlayGate = _OverlayChild9.w * step(0.1, overlayMask);
    float3 overlayScale = lerp(1.0.xxx, _OverlayChild9.rgb, overlayGate.xxx);
    float3 albedoMasked = baseColor * overlayScale;
