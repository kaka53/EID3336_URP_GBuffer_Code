#ifndef EID215543_215544_GBUFFER_INCLUDED
#define EID215543_215544_GBUFFER_INCLUDED
...
float3 DecodeOctNormal215543(uint packed)
{
...
float4 DecodePackedTangent215543(uint packed, float3 n)
{
...
Varyings215543 EID215543Vertex(Attributes215543 input)
{
    Varyings215543 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 normalOS = packedBasis ? DecodeOctNormal215543(packed) : input.packedNormal.xyz;
    float4 tangentOS = packedBasis ? DecodePackedTangent215543(packed, normalOS) : input.input2;
...
GBufferOutput215544 EID215544Fragment(Varyings215543 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215544 o;
    ...
    float wrapMask = lerp(1.0, nmSample.w, _P03.z) * wrap;
    ...
    float emissive = lerp(_P03.w, 0.0, loc3x);
