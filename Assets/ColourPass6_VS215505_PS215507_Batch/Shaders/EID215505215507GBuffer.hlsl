#ifndef EID215505_215507_GBUFFER_INCLUDED
#define EID215505_215507_GBUFFER_INCLUDED
...
float _EID215507MipBias;
float _Global75W;
float _LightMixY;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215505
{
...
float3 DecodeOctNormal215505(uint packed)
{
...
float4 DecodePackedTangent215505(uint packed, float3 n)
{
...
Varyings215505 EID215505Vertex(Attributes215505 input)
{
    Varyings215505 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215505(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215505(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215507 EID215507Fragment(Varyings215505 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215507 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvBase, _EID215507MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res25, sampler_Res25, uvNormal, _P03.y + _EID215507MipBias);
    float4 materialSample = SAMPLE_TEXTURE2D_BIAS(_Res27, sampler_Res27, uvNormal, _EID215507MipBias);
    ...
    float4 overlaySample = SAMPLE_TEXTURE2D_BIAS(_Res29, sampler_Res29, uvOverlay, _EID215507MipBias);
