#ifndef EID215500_215502_GBUFFER_INCLUDED
#define EID215500_215502_GBUFFER_INCLUDED
...
float _EID215502MipBias;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215500
{
...
float3 DecodeOctNormal215500(uint packed)
{
...
float4 DecodePackedTangent215500(uint packed, float3 n)
{
...
Varyings215500 EID215500Vertex(Attributes215500 input)
{
    Varyings215500 o;
    uint packed = asuint(input.positionPacked.w);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215500(packed) : float3(input.positionPacked.w, 0.0, 1.0);
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215500(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215502 EID215502Fragment(Varyings215500 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215502 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res31, sampler_Res31, uvBase, _EID215502MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res33, sampler_Res33, uvNormal, _P03.y + _EID215502MipBias);
    float4 materialSample = SAMPLE_TEXTURE2D_BIAS(_Res35, sampler_Res35, uvNormal, _EID215502MipBias);

    float2 packedN = float2(normalSample.w * normalSample.x, normalSample.y);
    float2 normalXY = packedN * 2.0 - 1.0;
    float faceSign = isFrontFace ? 1.0 : (_P01.w > 0.0 ? -1.0 : 1.0);
    float tangentZ = max(0.0, sqrt(saturate(1.0 - dot(normalXY, normalXY)))) * faceSign;
    normalXY *= _P00.x;
