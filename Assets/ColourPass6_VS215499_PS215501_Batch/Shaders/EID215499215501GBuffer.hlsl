#ifndef EID215499_215501_GBUFFER_INCLUDED
#define EID215499_215501_GBUFFER_INCLUDED
...
float _EID215501MipBias;
float _Global75W;
float _LightMixY;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215499
{
...
float3 DecodeOctNormal215499(uint packed)
{
...
float4 DecodePackedTangent215499(uint packed, float3 n)
{
...
float2 DeadzoneNormalXY215499(float2 encoded)
{
    float2 nxy = encoded * 2.0 - 1.0;
    return abs(nxy) < 0.012.xx ? 0.0.xx : nxy;
}

Varyings215499 EID215499Vertex(Attributes215499 input)
{
    Varyings215499 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215499(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215499(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215501 EID215501Fragment(Varyings215499 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215501 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res23, sampler_Res23, uvBase, _EID215501MipBias);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res25, sampler_Res25, uvNormal, _P03.y + _EID215501MipBias);

    float2 nxyRaw = DeadzoneNormalXY215499(normalSample.xy);
    ...
    float2 uvOverlay = lerp(input.uv0, input.uv1, _P25.x) * _P30.xy + _P30.zw + _P29.xy * _Global75W;
    float4 overlaySample = SAMPLE_TEXTURE2D_BIAS(_Res27, sampler_Res27, uvOverlay, _EID215501MipBias);
