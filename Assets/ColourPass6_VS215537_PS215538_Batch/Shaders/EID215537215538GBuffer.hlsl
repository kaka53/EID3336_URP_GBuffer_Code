#ifndef EID215537_215538_GBUFFER_INCLUDED
#define EID215537_215538_GBUFFER_INCLUDED
...
float3 DecodeOctNormal215537(uint packed)
{
...
float4 DecodePackedTangent215537(uint packed, float3 n)
{
...
Varyings215537 EID215537Vertex(Attributes215537 input)
{
    Varyings215537 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215537(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215537(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215538 EID215538Fragment(Varyings215537 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215538 o;
    ...
    float scanPhase = scanTime + _P28.x * instanceOrigin.x;
    float wrapped = scanPhase - _P27.y * floor(scanPhase / max(_P27.y, 1e-8));
    float band = saturate(abs(viewLocal.y - ((wrapped - _P27.y * 0.3) * _P27.x + _P28.z)) / max(_P27.z, 1e-8));
    float scanMask = saturate(_P27.w * (1.0 - band));
    scanMask *= scanMask;
    float luma = lerp(0.2, dot(baseColor, 0.333.xxx), _P28.y);
    ...
    float3 rimSrc = _P25.rgb;
    if (_P23.w > 0.5)
    {
        float3 viewDir = _ScanGlobals.w != 0.0
            ? UNITY_MATRIX_V[2].xyz
            : GetWorldSpaceNormalizeViewDir(input.positionWS);
        float rim = 1.0 - pow(saturate(dot(viewDir, input.normalWS) + _P24.x), _P24.y);
        rim *= lerp(1.0, input.input3.y, _P24.z);
        rimSrc = _P25.rgb * rim;
    }

    float3 scanColor = lerp(rimSrc, overlayRgb, _InstanceChild7.www);
    float3 litScan = scanColor * scanAmt * lerp(baseColor, 1.0.xxx, _P23.yyy);
    litScan *= (1.0 - _InstanceStateYZ.z);
    float globalScale = (1.0 - _P23.x) + _ScanGlobals.z * _P23.x;
