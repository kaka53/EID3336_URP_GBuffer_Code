#ifndef EID215506_215508_GBUFFER_INCLUDED
#define EID215506_215508_GBUFFER_INCLUDED
...
float _EID215508MipBias;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215506
{
...
float3 DecodeOctNormal215506(uint packed)
{
...
float4 DecodePackedTangent215506(uint packed, float3 n)
{
...
Varyings215506 EID215506Vertex(Attributes215506 input)
{
    Varyings215506 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215506(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215506(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215508 EID215508Fragment(Varyings215506 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215508 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float mip = _EID215508MipBias;
    float2 uvBase = lerp(input.uv0, input.uv1, _P02.w) * _P11.xy + _P11.zw;
    float4 baseSample = SAMPLE_TEXTURE2D_BIAS(_Res31, sampler_Res31, uvBase, mip);
    float2 uvNormal = lerp(input.uv0, input.uv1, _P03.x) * _P12.xy + _P12.zw;
    float4 normalSample = SAMPLE_TEXTURE2D_BIAS(_Res33, sampler_Res33, uvNormal, _P03.y + mip);
    ...
    float4 extraSample = SAMPLE_TEXTURE2D_BIAS(_Res35, sampler_Res35, uvNormal, mip);
