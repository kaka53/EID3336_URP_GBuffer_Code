...
cbuffer _5_6 : register(b10)
{
    ...
    column_major float4x4 _6_m6_Captured : packoffset(c24);
    ...
    float4 _6_m11_Captured : packoffset(c44);
...
{
    float4 _8_m0_Captured : packoffset(c0);
...
{
    float4x4 _EID4649ClipToWorld;
    float4 _EID4649CameraPositionWS;
    float4 _EID4649ScreenSize;
    float _EID4649UseLiveCamera;
};

float4x4 EID4649_GetClipToWorld()
{
    return (_EID4649UseLiveCamera > 0.5f) ? _EID4649ClipToWorld : _6_m6_Captured;
}

float4 EID4649_GetCameraPositionWS()
{
    return (_EID4649UseLiveCamera > 0.5f) ? _EID4649CameraPositionWS : _6_m11_Captured;
}

float4 EID4649_GetScreenSize()
{
    return (_EID4649UseLiveCamera > 0.5f) ? _EID4649ScreenSize : _8_m0_Captured;
}
...
void frag_main()
{
    ...
    float2 _273 = _262 * EID4649_GetScreenSize().zw;
    uint2 _274 = uint2(_262);
    float2 _276 = (_273 * 2.0f) - 1.0f.xx;
    float4 _279 = float4(_276, _15.SampleLevel(sampler_PointClamp, (_262 + 0.5f.xx) * EID4649_GetScreenSize().zw, 0.0f).x, 1.0f);
    _279.y = -_276.y;
    float4 _282 = mul(EID4649_GetClipToWorld(), _279);
    ...
    float4 _297 = _27.SampleLevel(sampler_PointClamp, _273, 0.0f);
    float2 _797;
    do
    {
        ...
        float3 _325 = _286 - float3(_317.x ? _21_m1[0].xyz.x : EID4649_GetCameraPositionWS().xyz.x, _317.y ? _21_m1[0].xyz.y : EID4649_GetCameraPositionWS().xyz.y, _317.z ? _21_m1[0].xyz.z : EID4649_GetCameraPositionWS().xyz.z);
        ...
        if (_336 > 0.0f)
        {
            ...
            if (_315 > 0)
            {
                ...
                float4 _451 = _24.SampleLevel(sampler_PointClamp, _273, 0.0f);
                float _452 = _451.x;
                float _533;
                ...
                else
                {
                    ...
                    {
                        float4 _507 = _22.GatherRed(sampler_PointClamp, _475 + (mul(_251[_495], _488) * _21_m5[_471])) - _431.xxxx;
            ...
            if (all(bool2(_567.x > 0.0f.xx.x, _567.y > 0.0f.xx.y)) && all(bool2(_567.x < 1.0f.xx.x, _567.y < 1.0f.xx.y)))
            {
                ...
                if (_592 >= 0.0f)
                {
                    ...
                    if (all(bool3(_609.x > 0.0f.xxx.x, _609.y > 0.0f.xxx.y, _609.z > 0.0f.xxx.z)) && all(bool3(_609.x < 1.0f.xxx.x, _609.y < 1.0f.xxx.y, _609.z < 1.0f.xxx.z)))
                    {
                        ...
                        float _697 = _668.y;
                        float _699 = _671.y;
                        _733 = ((((_694 * _695) * _25.SampleCmpLevelZero(sampler_LinearClampCompare, float3(_681 + float2(_682, _683), _255).xy, _626)) + ((_697 * _695) * _25.SampleCmpLevelZero(sampler_LinearClampCompare, float3(_681 + float2(_686, _683), _255).xy, _626))) + ((_694 * _699) * _25.SampleCmpLevelZero(sampler_LinearClampCompare, float3(_681 + float2(_682, _689), _255).xy, _626))) + ((_697 * _699) * _25.SampleCmpLevelZero(sampler_LinearClampCompare, float3(_681 + float2(_686, _689), _255).xy, _626));
                    }
                    else
                    {
                        _733 = 1.0f;
        ...
        if (_741 > 0.001000000047497451305389404296875f)
        {
            ...
            _792 = _741 * lerp(1.0f, lerp(_23.SampleLevel(sampler_LinearRepeat, _758 + _767, 0.0f), _23.SampleLevel(sampler_LinearRepeat, (_758 * _8_m67.w) + _767, 0.0f), smoothstep(_8_m66.x, _8_m66.y, length(_748.xz)).xxxx).x, _8_m67.z);
        }
        else
        {
            _792 = _741;
