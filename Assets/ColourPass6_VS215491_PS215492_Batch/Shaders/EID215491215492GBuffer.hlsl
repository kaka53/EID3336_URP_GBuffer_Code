#ifndef EID215491_215492_GBUFFER_INCLUDED
#define EID215491_215492_GBUFFER_INCLUDED
...
float4 _P24; float4 _P25;
float4 _InstanceMeta;
float4 _InstanceStateYZ;
float _EID215492MipBias;
float _UseBakedSkinning;
CBUFFER_END

struct Attributes215491
{
...
float3 DecodeOctNormal215491(uint packed)
{
...
float4 DecodePackedTangent215491(uint packed, float3 n)
{
...
Varyings215491 EID215491Vertex(Attributes215491 input)
{
    Varyings215491 o;
    uint packed = asuint(input.packedNormal.x);
    bool packedBasis = (packed & 0x40000000u) != 0u;
    float3 decodedNormalOS = packedBasis ? DecodeOctNormal215491(packed) : input.packedNormal.xyz;
    float4 decodedTangentOS = packedBasis ? DecodePackedTangent215491(packed, decodedNormalOS) : input.input2;
...
GBufferOutput215492 EID215492Fragment(Varyings215491 input, bool isFrontFace : SV_IsFrontFace)
{
    GBufferOutput215492 o;
    float tangentInputSign = input.tangentWS.w > 0.0 ? 1.0 : -1.0;
    float mip = _EID215492MipBias;
    ...
    float2 uvExtra = lerp(input.uv0, input.uv1, _P23.z) * _P25.xy + _P25.zw;
    float4 extraSample = SAMPLE_TEXTURE2D_BIAS(_Res31, sampler_Res31, uvExtra, mip);
    float extraSrc = lerp(1.0, extraSample.w, saturate(_P22.y));
    float extraAlt = lerp(lerp(albedoSample.w, normalSample.z, saturate(_P22.y - 2.0)), normalSample.w, saturate(_P22.y - 3.0));
    float extraMask = lerp(extraSrc, extraAlt, saturate(_P22.y - 1.0));
    float viewAbsZ = abs(TransformWorldToView(input.positionWS).z);
    extraMask *= saturate((_P23.y - viewAbsZ) / max(_P23.y - _P23.x, 1e-8));

    float2 exyRaw = DeadzoneNormalXY(extraSample.xy);
    float2 exy = exyRaw * (extraMask * _P22.z);
    float3 extraTS = float3(exy, sqrt(saturate(1.0 - dot(exyRaw, exyRaw))));
    ...
    float extraBlend = _P22.w * extraMask;
    roughness = lerp(roughness, lerp(extraSample.w, extraSample.z, _P22.x), extraBlend);
    ao *= lerp(1.0, lerp(1.0, extraSample.w, _P22.x), extraBlend);
    float extraColorAmt = extraMask * (1.0 - _P22.x) * (1.0 - extraSample.z);
    float3 extraCol = lerp(saturate(baseColor * _P24.rgb * _P23.w), _P24.rgb, _P24.aaa);
